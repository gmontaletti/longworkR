...
add_contract_survival_metrics <- function(
  ...
  if (!contract_type_var %in% names(data)) {
    stop(sprintf(
      "Contract type variable '%s' not found in data",
      contract_type_var
    ))
  }
...
estimate_contract_survival <- function(
  ...
  for (ct in contract_types) {
    ...
    if (nrow(ct_data) == 0) {
      warning(sprintf(
        "No data available for contract type '%s'. Skipping.",
        ct
      ))
      next
    }
    ...
    invalid_duration <- is.na(ct_data[[duration_var]]) |
      ct_data[[duration_var]] <= 0
    if (any(invalid_duration)) {
      warning(sprintf(
        "Contract type '%s' has %d invalid duration values. Removing them.",
        ct,
      ...
      if (nrow(ct_data) == 0) {
        warning(sprintf(
          "No valid data remaining for contract type '%s' after removing invalid durations. Skipping.",
          ct
        ))
        next
      }
    ...
    km_fit <- tryCatch(
      ...
      error = function(e) {
        warning(sprintf(
          "Failed to fit survival curve for contract type '%s': %s",
          ct,
          e$message
        ))
        return(NULL)
    ...
    median_survival[ct] <- tryCatch(
      {
        ...
        if (is.na(median_result) || is.infinite(median_result)) {
          ...
          if (min_survival > 0.5) {
            ...
            message(sprintf(
              "Contract type '%s': Median survival time cannot be estimated (min survival probability: %.3f > 0.5, censoring rate: %.1f%%)",
              ct,
              min_survival,
              censoring_rate * 100
            ))
            ...
            if (length(surv_below_half) > 0) {
              ...
              message(sprintf(
                "Contract type '%s': Median estimated using manual calculation (%.2f days)",
                ct,
                median_manual
              ))
              median_manual
          ...
          message(sprintf(
            "Contract type '%s': Median survival time estimated at %.2f days",
            ct,
            median_result
          ))
          median_result
        ...
        tryCatch(
          {
            ...
            if (length(surv_below_half) > 0) {
              median_manual <- km_fit$time[surv_below_half[1]]
              warning(sprintf(
                "Contract type '%s': Quantile function failed, using manual calculation (%.2f days): %s",
                ct,
                median_manual,
                e$message
              ))
              ...
              censoring_rate <- mean(ct_data[[censored_var]])
              warning(sprintf(
                "Contract type '%s': Cannot estimate median (censoring rate: %.1f%%, quantile error: %s)",
                ct,
                censoring_rate * 100,
                e$message
              ))
          ...
          error = function(e2) {
            # Complete fallback
            warning(sprintf(
              "Unable to estimate median for contract type '%s': %s",
              ct,
              e2$message
            ))
            NA_real_
    ...
    confidence_intervals[[ct]] <- tryCatch(
      {
        ...
        list(
          median = if (is.na(median_ci$quantile)) {
            NA_real_
          } else {
            median_ci$quantile
          },
      ...
      if (any(ct_mask)) {
        ...
        if (ct %in% names(confidence_intervals)) {
          ...
          if (!is.null(ci)) {
            result[
              ct_mask,
              median_ci_lower := if (!is.null(ci$lower)) ci$lower else NA_real_
            ]
            result[
              ct_mask,
              median_ci_upper := if (!is.null(ci$upper)) ci$upper else NA_real_
            ]
          }
        }
      }
...
calculate_median_survival_probability <- function(
  ...
  warning(
    "calculate_median_survival_probability() is deprecated. The median survival probability calculation was flawed. Use add_contract_survival_metrics() instead for proper median survival times with confidence intervals."
  )
...
compare_contract_survival <- function(
  ...
  if (n_groups < 2) {
    stop(sprintf(
      "Need at least 2 groups for comparison. Found %d groups: %s",
      n_groups,
  ...
  cat(sprintf(
    "Comparing survival across %d contract types with %d observations:\n",
    n_groups,
    nrow(complete_data)
  ))
    ...
    for (i in 1:(n_types - 1)) {
      for (j in (i + 1):n_types) {
        pair_data <- complete_data[
          get(contract_type_var) %in% c(contract_types[i], contract_types[j])
        ]

        if (nrow(pair_data) == 0) {
          warning(sprintf(
            "No data available for pairwise comparison between %s and %s",
            contract_types[i],
            contract_types[j]
          ))
          next
        ...
        pair_test <- tryCatch(
          ...
          error = function(e) {
            warning(sprintf(
              "Failed pairwise comparison between %s and %s: %s",
              contract_types[i],
              contract_types[j],
              e$message
            ))
