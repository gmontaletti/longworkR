...
#'
#' @keywords internal
.create_empty_statistics_result <- function(
...
.calculate_weighted_median_optimized <- function(
  values,
  weights,
  na.rm = TRUE
) {
...
#' \code{consolidation_mode} parameter, or users can pre-consolidate data using
#' the composable consolidation functions (\code{consolidate_overlapping()},
#' \code{consolidate_adjacent()}, \code{consolidate_short_gaps()}).
#'
#' @details
...
#' @param consolidation_mode Character string specifying the consolidation strategy
#'   to apply before transition analysis (default: "none"). Options:
#'   \itemize{
#'     \item{\code{"none"}}: No consolidation (default, backward-compatible)
#'     \item{\code{"temporal"}}: Consolidate overlapping periods using over_id
#'     \item{\code{"employer"}}: Consolidate consecutive contracts from the same employer
#'     \item{\code{"both"}}: Apply employer consolidation first, then temporal
#'   }
#' @param consolidation_type Character string specifying the temporal consolidation
#'   approach (default: "both"). Only used when \code{consolidation_mode} includes
#'   temporal consolidation. Options: "both", "overlapping", "employment_only".
#' @param employer_var Character string specifying the column name containing
#'   employer identifiers. Required when \code{consolidation_mode} is "employer"
#'   or "both". Ignored for other modes (with a warning if provided).
#' @param min_lag Numeric value specifying the maximum gap in days between
#'   consecutive contracts from the same employer to be consolidated (default: 8).
#'   Only used with employer-based consolidation modes.
...
#' }
analyze_employment_transitions <- function(
  pipeline_result,
  ...
  if (!inherits(pipeline_result, "data.table")) {
    stop(
      "Parameter 'pipeline_result' must be a data.table object from process_employment_pipeline()."
    )
  }
  ...
  if (!is.numeric(min_unemployment_duration) || min_unemployment_duration < 0) {
    stop(
      "Parameter 'min_unemployment_duration' must be a non-negative numeric value."
    )
  }

  if (!is.null(max_unemployment_duration)) {
    if (
      !is.numeric(max_unemployment_duration) || max_unemployment_duration < 0
    ) {
      stop(
        "Parameter 'max_unemployment_duration' must be NULL or a non-negative numeric value."
      )
    }
    if (max_unemployment_duration < min_unemployment_duration) {
      stop(
        "Parameter 'max_unemployment_duration' must be greater than or equal to 'min_unemployment_duration'."
      )
    }
  }

  if (
    !is.logical(output_transition_matrix) ||
      length(output_transition_matrix) != 1
  ) {
    stop(
      "Parameter 'output_transition_matrix' must be a single logical value (TRUE or FALSE)."
    )
  }
  ...
  if (output_transition_matrix && length(statistics_variables) > 0) {
    if (show_progress) {
      message(sprintf(
        "Note: output_transition_matrix = TRUE, disabling statistics calculation for %d variables: %s",
  ...
  if (!eval_chain %in% valid_eval_chain) {
    stop(paste(
      "Parameter 'eval_chain' must be one of:",
      paste(valid_eval_chain, collapse = ", ")
    ))
  }

  # 1. Validate consolidation parameters -----
  valid_consolidation_modes <- c("none", "temporal", "employer", "both")
  if (!consolidation_mode %in% valid_consolidation_modes) {
    stop(paste(
      "Parameter 'consolidation_mode' must be one of:",
      paste(valid_consolidation_modes, collapse = ", ")
    ))
  }
  ...
  if (consolidation_mode %in% c("employer", "both")) {
    if (is.null(employer_var)) {
      stop(sprintf(
        "Parameter 'employer_var' is required when consolidation_mode = '%s'",
        consolidation_mode
      ))
    }
    ...
    if (!employer_var %in% names(pipeline_result)) {
      stop(sprintf("Column %s not found in pipeline_result", employer_var))
    }
  }

  if (!is.null(employer_var) && consolidation_mode %in% c("none", "temporal")) {
    warning(sprintf(
      "Parameter 'employer_var' is ignored when consolidation_mode = '%s'",
      consolidation_mode
    ))
  }
  ...
  if (!consolidation_type %in% valid_consolidation_types) {
    stop(paste(
      "Parameter 'consolidation_type' must be one of:",
  ...
  if (
    ...
    if (show_progress) {
      message(
        "Note: 'over_id' column not found. Temporal consolidation requires ",
        "over_id from vecshift output. Skipping temporal consolidation."
      )
    }
  ...
  if (length(available_cols) == 0) {
    stop(
      "No additional columns found for analysis. Pipeline result should contain variables beyond standard vecshift output columns."
    )
  }
  ...
  if (is.null(transition_variable)) {
    ...
    if (show_progress) {
      message(sprintf(
        "Using '%s' as transition variable (first available attribute)",
        transition_variable
      ))
    }
  }
  ...
  if (!transition_variable %in% names(pipeline_result)) {
    stop(sprintf(
      "Transition variable '%s' not found in pipeline_result.",
      transition_variable
    ))
  }
  ...
  if (is.null(statistics_variables)) {
    ...
    if (show_progress && length(statistics_variables) > 0) {
      message(sprintf(
        "Using %d variables for statistics: %s",
        length(statistics_variables),
        paste(statistics_variables, collapse = ", ")
      ))
    }
  ...
  if (length(missing_stats_cols) > 0) {
    stop(paste(
      "Statistics variables not found in pipeline_result:",
  ...
  .apply_temporal_consolidation <- function(
    data,
    consolidation_type,
    show_progress = FALSE
  ) {
    ...
    if (
      ...
      return(merge_consecutive_employment_fast_with_type(
        data_converted,
      ...
      if (show_progress) {
        message(
          "Using standard (slower) consolidation method - consider updating vecshift package"
        )
      }
      return(merge_consecutive_employment(
        data_converted,
        consolidation_type = consolidation_type
      ))
    } else {
      stop(
        "No consolidation function found. Ensure the vecshift package is properly loaded."
      )
    }
  }
  ...
  # 2. Apply consolidation if requested -----
  if (consolidation_mode != "none") {
    if (show_progress) {
      message(sprintf("Applying %s consolidation...", consolidation_mode))
    }
  ...
  if (length(multi_period_persons) == 0) {
    if (show_progress) {
      message(
        "No persons found with sufficient periods for transition analysis."
      )
    }

    # OPTIMIZED: Use helper function to create empty result structure (eliminates code duplication)
    return(.create_empty_statistics_result(
      statistics_variables,
      pipeline_result,
  ...
  if (is.null(max_unemployment_duration)) {
    dt[,
      is_transition_unemployment := (
        !is.na(prev_arco) &
          prev_arco >= 1 & # Previous: employment
          !is.na(arco) &
          arco == 0 & # Current: unemployment
          !is.na(next_arco) &
          next_arco >= 1 & # Next: employment
          !is.na(durata) &
          durata >= min_unemployment_duration # Min duration
      )
    ]
  } else {
    dt[,
      is_transition_unemployment := (
        !is.na(prev_arco) &
          ...
          durata >= min_unemployment_duration & # Min duration
          !is.na(durata) &
          durata <= max_unemployment_duration # Max duration
      )
    ]
  }
  ...
  # Create subset
  col_subset <- transition_periods[,
    c(required_cols, existing_stat_cols),
    with = FALSE
  ]
  ...
  setnames(
    col_subset,
    c(from_transition_col, to_transition_col),
    c("from", "to")
  )
  ...
  for (stat_var in statistics_variables) {
    ...
    if (
      from_stat_col %in%
        names(transitions_data) &&
        to_stat_col %in% names(transitions_data)
    ) {
      if (is.numeric(pipeline_result[[stat_var]])) {
        ...
        stat_agg <- transitions_data[,
          {
            from_value <- {
              w <- as.numeric(from_durata[
                !is.na(get(from_stat_col)) & !is.na(from_durata)
              ])
              v <- as.numeric(get(from_stat_col))[
                !is.na(get(from_stat_col)) & !is.na(from_durata)
              ]
            ...
            to_value <- {
              w <- as.numeric(to_durata[
                !is.na(get(to_stat_col)) & !is.na(to_durata)
              ])
              v <- as.numeric(get(to_stat_col))[
                !is.na(get(to_stat_col)) & !is.na(to_durata)
              ]
              if (length(w) > 0 && sum(w, na.rm = TRUE) > 0) {
                # OPTIMIZED: Use matrixStats::weightedMedian instead of rep() trick
        ...
        stat_agg <- transitions_data[,
          {
            if (use_collapse) {
              ...
              from_value <- .calculate_mode_optimized(
                get(from_stat_col),
                na.rm = TRUE
              )
              to_value <- .calculate_mode_optimized(
                get(to_stat_col),
                na.rm = TRUE
              )
            }
  ...
  if (show_progress) {
    ...
    message(sprintf(
      "Transition analysis completed in %.2f seconds. Found %d unique transition patterns with %d total transitions.",
...
#' }
analyze_consolidated_periods <- function(
  segments,
  level = "both",
  include_unemployment = TRUE,
  consolidation_type = "both",
  ...
  if (!inherits(segments, "data.table")) {
    stop(
      "Parameter 'segments' must be a data.table object from vecshift() output."
    )
  }
  ...
  if (!level %in% valid_levels) {
    stop(paste(
      "Parameter 'level' must be one of:",
  ...
  if (length(missing_cols) > 0) {
    stop(paste(
      "Missing required columns in segments:",
      paste(missing_cols, collapse = ", ")
    ))
  }
  ...
  if (!"over_id" %in% names(segments)) {
    stop(
      "Parameter 'segments' must contain 'over_id' column from vecshift() output. ",
  ...
  if (!is.numeric(min_employment_duration) || min_employment_duration < 0) {
    stop(
      "Parameter 'min_employment_duration' must be a non-negative numeric value."
    )
  }
  ...
  if (!is.logical(include_unemployment) || !is.logical(include_details)) {
    stop(
      "Parameters 'include_unemployment' and 'include_details' must be logical values."
    )
  }
  ...
  validation_results <- list(
    input_validation = list(
      ...
      date_range = if (nrow(dt) > 0) {
        c(min(dt$inizio, na.rm = TRUE), max(dt$fine, na.rm = TRUE))
      } else {
        c(NA, NA)
      },
      has_employment_periods = any(dt$arco >= 1, na.rm = TRUE),
      has_unemployment_periods = any(dt$arco == 0, na.rm = TRUE),
      over_id_range = if (nrow(dt) > 0) {
        c(min(dt$over_id, na.rm = TRUE), max(dt$over_id, na.rm = TRUE))
      } else {
        c(NA, NA)
      }
    )
    ...
    person_stats <- dt[,
      {
        ...
        consolidated_unemployment <- if (include_unemployment) {
          uniqueN(over_id[over_id == 0])
        } else {
          0
        }
        total_consolidated <- consolidated_employment +
          consolidated_unemployment

        # Employment complexity metrics
        employment_complexity_avg <- if (employment_original > 0) {
          mean(arco[arco >= 1], na.rm = TRUE)
        } else {
          0
        }
        max_simultaneous_jobs <- if (employment_original > 0) {
          max(arco[arco >= 1], na.rm = TRUE)
        } else {
          0
        }
        ...
        avg_employment_duration_original <- if (employment_original > 0) {
          mean(durata[arco >= 1], na.rm = TRUE)
        } else {
          0
        }
        ...
        if (consolidated_employment > 0) {
          consolidated_durations <- dt[
            cf == .BY$cf & over_id > 0,
            .(consolidated_duration = sum(durata)),
            by = over_id
          ]
          total_employment_days_consolidated <- sum(
            consolidated_durations$consolidated_duration,
            na.rm = TRUE
          )
          avg_employment_duration_consolidated <- mean(
            consolidated_durations$consolidated_duration,
            na.rm = TRUE
          )
        } else {
        ...
        consolidation_ratio <- if (total_original > 0) {
          1 - (total_consolidated / total_original)
        } else {
          0
        }
        duration_efficiency <- if (total_employment_days_original > 0) {
          total_employment_days_consolidated / total_employment_days_original
        } else {
          1
        }
        ...
        if (consolidated_employment > 0) {
          overlapping_episodes <- dt[
            cf == .BY$cf & over_id > 0,
            .N,
            by = over_id
          ][N > 1, .N]
          overlapping_episodes <- ifelse(
            length(overlapping_episodes) == 0,
            0,
        ...
        list(
          ...
          unemployment_periods = if (include_unemployment) {
            consolidated_unemployment
          } else {
          ...
          avg_employment_duration_original = round(
            avg_employment_duration_original,
            1
          ),
          avg_employment_duration_consolidated = round(
            avg_employment_duration_consolidated,
            1
          ),
  ...
  if (level %in% c("aggregate", "both")) {
    ...
    aggregate_stats <- list(
      ...
      total_consolidated_employment_periods = uniqueN(dt[
        over_id > 0,
        .(cf, over_id)
      ]),
      total_consolidated_unemployment_periods = if (include_unemployment) {
        uniqueN(dt[over_id == 0, .(cf, over_id)])
      } else {
        0
      },
      avg_periods_per_person_original = round(nrow(dt) / uniqueN(dt$cf), 2),
      avg_employment_periods_per_person = round(
        sum(dt$arco >= 1, na.rm = TRUE) / uniqueN(dt$cf),
        2
      ),
      median_employment_duration = round(
        median(dt[arco >= 1, durata], na.rm = TRUE),
        1
      ),
      avg_employment_complexity = round(
        mean(dt[arco >= 1, arco], na.rm = TRUE),
        2
      ),
      max_employment_complexity = max(dt[arco >= 1, arco], na.rm = TRUE),
      pct_with_overlapping_employment = round(
        100 * uniqueN(dt[arco > 1, cf]) / uniqueN(dt$cf),
        1
      )
    )
    ...
    if (nrow(dt) > 0 && any(dt$over_id > 0)) {
      ...
      aggregate_stats$overall_consolidation_ratio <- round(
        1 - (total_consolidated / total_original),
        4
      )
      aggregate_stats$consolidation_efficiency_pct <- round(
        100 * (1 - total_consolidated / total_original),
        1
      )
    } else {
    ...
    consolidation_benefits <- dt[
      over_id > 0,
      {
        ...
        list(
          ...
          administrative_efficiency = if (segments_in_period > 1) {
            "High"
          } else {
            "Standard"
          }
  ...
  if (level == "person") {
    return(list(
      ...
      consolidation_benefits = if (include_details) {
        results$consolidation_benefits
      } else {
        NULL
      },
      employment_complexity = if (include_details) {
        results$employment_complexity
      } else {
        NULL
      }
    ))
...
.calculate_consolidation_impact <- function(dt, person_id = NULL) {
  ...
  if (any(dt$arco >= 1)) {
    ...
    impact_score <- consolidation_ratio *
      (1 + log10(1 + avg_employment_complexity)) *
      log10(1 + total_employment_days)
  } else {
    impact_score <- consolidation_ratio
  }
...
.validate_over_id_for_analysis <- function(dt) {
  validation <- list(
    has_over_id = "over_id" %in% names(dt),
    over_id_completeness = if ("over_id" %in% names(dt)) {
      sum(!is.na(dt$over_id)) / nrow(dt)
    } else {
      0
    },
    unemployment_properly_coded = if ("over_id" %in% names(dt)) {
      all(dt[arco == 0, over_id] == 0, na.rm = TRUE)
    } else {
      FALSE
    },
    employment_properly_coded = if ("over_id" %in% names(dt)) {
      all(dt[arco >= 1, over_id] > 0, na.rm = TRUE)
    } else {
      FALSE
    },
    over_id_range = if ("over_id" %in% names(dt)) {
      range(dt$over_id, na.rm = TRUE)
    } else {
      c(NA, NA)
    },
    persons_with_overlapping = if ("over_id" %in% names(dt)) {
      uniqueN(dt[arco > 1, cf])
    } else {
      0
    },
    total_over_id_groups = if ("over_id" %in% names(dt)) {
      uniqueN(dt$over_id)
    } else {
      0
    }
  ...
  validation$quality_grade <- if (validation$quality_score >= 0.95) {
    "Excellent"
  } else if (validation$quality_score >= 0.85) {
    "Good"
  } else if (validation$quality_score >= 0.70) {
    "Fair"
  } else {
    "Poor"
  }
...
#' }
create_consolidated_transition_matrix <- function(
  pipeline_result,
  transition_variable,
  consolidation_type = "both",
  ...
  if (!inherits(pipeline_result, "data.table")) {
    stop(
      "Parameter 'pipeline_result' must be a data.table object from vecshift() or process_employment_pipeline()."
    )
  }
  ...
  if (!"over_id" %in% names(pipeline_result)) {
    stop(
      "Parameter 'pipeline_result' must contain 'over_id' column from vecshift() output. ",
  ...
  if (!is.logical(include_comparison) || !is.logical(show_progress)) {
    stop(
      "Parameters 'include_comparison' and 'show_progress' must be logical values."
    )
  }

  if (show_progress) {
    message(sprintf(
      "Creating consolidated transition matrix for '%s' using '%s' consolidation...",
      transition_variable,
      consolidation_type
    ))
  }
  ...
  if (
    is.null(consolidated_transitions) ||
      (is.matrix(consolidated_transitions) &&
        nrow(consolidated_transitions) == 0)
  ) {
    if (show_progress) {
  ...
  if ("probability" %in% matrix_type) {
    consolidated_prob_matrix <- .normalize_transition_matrix(
      consolidated_freq_matrix,
      normalize_by
    )
  ...
  if (length(matrix_type) > 1) {
    results$consolidated_frequency_matrix <- consolidated_freq_matrix
    if ("probability" %in% matrix_type) {
      results$consolidated_probability_matrix <- .normalize_transition_matrix(
        consolidated_freq_matrix,
        normalize_by
      )
    }
  ...
  matrix_stats <- .calculate_matrix_statistics(
    consolidated_freq_matrix,
    "consolidated"
  )
  ...
  if (include_comparison && consolidation_type != "none") {
    ...
    if (!is.null(raw_transitions) && nrow(raw_transitions) > 0) {
      ...
      consolidation_impact <- .calculate_consolidation_impact_matrix(
        raw_transitions,
        consolidated_freq_matrix
      )
  ...
  if (show_progress) {
    ...
    sparsity <- round(
      100 *
        sum(consolidated_freq_matrix == 0) /
        length(consolidated_freq_matrix),
      1
    )

    message(
      sprintf(
        "Matrix creation completed in %.2f seconds. ",
        as.numeric(elapsed_time)
      ),
      sprintf(
        "%d states, %d total transitions, %.1f%% sparsity",
        n_states,
        total_transitions,
        sparsity
      )
  ...
  setattr(
    ...
    list(
      consolidation_type = consolidation_type,
      transition_variable = transition_variable,
      matrix_dimensions = if (!is.null(consolidated_freq_matrix)) {
        dim(consolidated_freq_matrix)
      } else {
        c(0, 0)
      },
      total_transitions = if (!is.null(consolidated_freq_matrix)) {
        sum(consolidated_freq_matrix, na.rm = TRUE)
      } else {
        0
      }
    )
...
#' @keywords internal
#' @noRd
.normalize_transition_matrix_optimized <- function(
  matrix,
  normalize_by = "row"
) {
...
.calculate_matrix_statistics <- function(matrix, matrix_name) {
  ...
  pct_self_transitions <- if (total_transitions > 0) {
    n_self_transitions / total_transitions
  } else {
    0
  }
  ...
  statistics <- data.table(
    ...
    metric = c(
      ...
      "density",
      "n_self_transitions",
      "pct_self_transitions",
      "active_from_states",
      "active_to_states",
      "mean_transition_weight",
      "median_transition_weight",
    ...
    value = c(
      ...
      round(1 - sparsity, 4),
      n_self_transitions,
      round(pct_self_transitions, 4),
      active_from_states,
      active_to_states,
      round(mean_transition_weight, 2),
      round(median_transition_weight, 2),
      max_transition_weight
    )
  )
...
#' @keywords internal
#' @noRd
.compare_transition_matrices <- function(
  raw_matrix,
  consolidated_matrix,
  raw_stats,
  consolidated_stats
  ...
  comparison_data <- data.table(
    ...
    consolidated_value = sapply(metrics, function(m) {
      extract_metric(consolidated_stats, m)
    })
  )
  ...
  comparison_data[,
    improvement := ifelse(
      metric == "sparsity",
      raw_value - consolidated_value, # Lower sparsity is better
      ifelse(
        metric == "n_self_transitions",
        raw_value - consolidated_value, # Fewer self-transitions is better
        ifelse(
          metric == "pct_self_transitions",
          raw_value - consolidated_value, # Lower % self-transitions is better
          ifelse(
            metric == "density",
            consolidated_value - raw_value, # Higher density is better
            ifelse(
              metric %in% c("n_states", "total_transitions", "n_nonzero_cells"),
              raw_value - consolidated_value, # Simplification is better
              0
            )
          )
        )
  ...
  comparison_data[,
    improvement_pct := round(100 * improvement / pmax(raw_value, 1e-10), 1)
  ]

  comparison_data[,
    interpretation := ifelse(
      metric == "sparsity" & improvement > 0,
      paste0("Reduced sparsity by ", round(improvement, 4)),
      ifelse(
        metric == "n_self_transitions" & improvement > 0,
        paste0("Eliminated ", improvement, " administrative self-transitions"),
        ifelse(
          metric == "pct_self_transitions" & improvement > 0,
          paste0(
            "Reduced self-transition rate by ",
            round(100 * improvement, 1),
            "%"
          ),
          ifelse(
            metric == "density" & improvement > 0,
            paste0("Increased matrix density by ", round(improvement, 4)),
            ifelse(
              metric == "n_states" & improvement != 0,
              ifelse(
                improvement > 0,
...
#' @keywords internal
#' @noRd
.calculate_consolidation_impact_matrix <- function(
  raw_matrix,
  consolidated_matrix
) {
  ...
  administrative_noise_removed <- raw_self_transitions -
    consolidated_self_transitions
  transition_reduction_pct <- if (raw_transitions > 0) {
    round(
      100 * (raw_transitions - consolidated_transitions) / raw_transitions,
      1
    )
  } else {
    0
  ...
  impact_summary <- data.table(
    ...
    interpretation = c(
      paste(
        raw_transitions - consolidated_transitions,
        "transitions consolidated into cleaner patterns"
      ),
      paste(
        administrative_noise_removed,
        "administrative self-transitions eliminated"
      ),
      paste0(
        transition_reduction_pct,
        "% reduction in total transition volume"
      ),
      paste0(
        self_transition_reduction_pct,
        "% reduction in self-transition noise"
      ),
      ifelse(
        sparsity_improvement < 0,
        paste(
          "Matrix density improved by",
          round(abs(sparsity_improvement), 4)
        ),
        paste("Matrix sparsity reduced by", round(sparsity_improvement, 4))
      ),
      paste0(
        round(100 * administrative_noise_removed / pmax(raw_transitions, 1), 1),
        "% efficiency gain from noise reduction"
      )
    )
  )
...
#' }
analyze_employment_overlaps <- function(
  segments,
  analysis_level = "person",
  min_overlap_duration = 1,
  include_temporal_patterns = TRUE,
  ...
  if (!analysis_level %in% valid_levels) {
    stop(paste(
      "Parameter 'analysis_level' must be one of:",
      paste(valid_levels, collapse = ", ")
    ))
  }
  ...
  if (!temporal_aggregation %in% valid_temporal) {
    stop(paste(
      "Parameter 'temporal_aggregation' must be one of:",
      paste(valid_temporal, collapse = ", ")
    ))
  }
  ...
  if (!is.numeric(min_overlap_duration) || min_overlap_duration < 0) {
    stop(
      "Parameter 'min_overlap_duration' must be a non-negative numeric value."
    )
  }

  # Validate logical parameters
  if (
    !is.logical(include_temporal_patterns) ||
      !is.logical(include_unemployment_context)
  ) {
    stop(
      "Parameters 'include_temporal_patterns' and 'include_unemployment_context' must be logical values."
    )
  }
  ...
  overlap_validation <- list(
    input_validation = list(
      ...
      arco_range = if (nrow(dt) > 0) {
        c(min(dt$arco, na.rm = TRUE), max(dt$arco, na.rm = TRUE))
      } else {
        c(NA, NA)
      },
      over_id_range = if (nrow(dt) > 0) {
  ...
  if (nrow(dt) == 0) {
    warning("No records remaining after filtering. Returning empty results.")
    empty_result <- .create_empty_overlap_results(
      analysis_level,
      group_by,
  ...
  if (sum(dt$arco > 1, na.rm = TRUE) == 0) {
    message(
      "No overlapping employment periods found (no periods with arco > 1)."
    )
    ...
    results <- .create_no_overlap_results(
      analysis_level,
      dt,
      person_results,
      group_by,
  ...
  # Person-level analysis
  if (analysis_level %in% c("person", "detailed")) {
    person_overlaps <- .calculate_person_level_overlaps(
      dt,
  ...
  if (analysis_level %in% c("aggregate", "detailed")) {
    aggregate_overlaps <- .calculate_aggregate_overlaps(
      dt,
      include_unemployment_context
    )
  ...
  if (analysis_level == "detailed") {
    ...
    employment_intensity_dist <- .calculate_employment_intensity_distribution(
      dt
    )
    ...
    if (include_temporal_patterns) {
      temporal_patterns <- .calculate_temporal_overlap_patterns(
        dt,
    ...
    if (!is.null(group_by)) {
      group_comparisons <- .calculate_group_overlap_comparisons(
        dt,
        group_by,
        include_unemployment_context
      )
...
#' @keywords internal
#' @noRd
.calculate_person_level_overlaps <- function(
  dt,
  include_unemployment_context = TRUE
) {
  person_overlaps <- dt[,
    {
      ...
      overlap_frequency <- if (total_employment_periods > 0) {
        overlapping_episodes / total_employment_periods
      } else {
        0
      }
      ...
      overlap_intensity_pct <- if (total_employment_days > 0) {
        (total_overlap_days / total_employment_days) * 100
      } else {
        0
      }
      ...
      if (total_employment_days > 0) {
        ...
        employment_complexity_score <- sum(complexity_weights, na.rm = TRUE) /
      ...
      if (include_unemployment_context) {
        ...
        employment_to_unemployment_ratio <- if (total_unemployment_days > 0) {
          total_employment_days / total_unemployment_days
        } else {
          Inf
        }
      } else {
      ...
      list(
        ...
        employment_to_unemployment_ratio = if (
          is.infinite(employment_to_unemployment_ratio)
        ) {
          NA_real_
        } else {
          round(employment_to_unemployment_ratio, 2)
        }
      )
    },
    by = cf
...
#' @keywords internal
#' @noRd
.calculate_aggregate_overlaps <- function(
  dt,
  include_unemployment_context = TRUE
) {
  # Population-level aggregate statistics
  ...
  pct_persons_with_overlaps <- round(
    100 * persons_with_overlaps / total_persons,
    1
  )

  # Employment intensity metrics
  overlap_frequency_population <- if (total_employment_periods > 0) {
    total_overlapping_periods / total_employment_periods
  } else {
    0
  }
  ...
  total_employment_days <- sum(dt[arco >= 1, durata], na.rm = TRUE)
  population_overlap_intensity <- if (total_employment_days > 0) {
    (total_overlap_days / total_employment_days) * 100
  } else {
    0
  }
  ...
  # Overlap episode characteristics
  avg_overlap_duration <- if (total_overlapping_periods > 0) {
    mean(dt[arco > 1, durata], na.rm = TRUE)
  } else {
    0
  }
  median_overlap_duration <- if (total_overlapping_periods > 0) {
    median(dt[arco > 1, durata], na.rm = TRUE)
  } else {
    0
  }
  ...
  avg_overlap_episodes_per_person <- if (persons_with_overlaps > 0) {
    unique_overlap_episodes / persons_with_overlaps
  } else {
    0
  }
  ...
  if (include_unemployment_context) {
    ...
    employment_to_unemployment_days_ratio <- if (total_unemployment_days > 0) {
      total_employment_days / total_unemployment_days
    } else {
      Inf
    }
    ...
    aggregate_stats$employment_to_unemployment_ratio <- if (
      is.infinite(employment_to_unemployment_days_ratio)
    ) {
      NA_real_
    } else {
      round(employment_to_unemployment_days_ratio, 2)
    }
  }
...
.calculate_overlap_duration_distribution <- function(dt) {
  ...
  duration_dist[,
    duration_category := factor(duration_category, levels = duration_order)
  ]
...
.calculate_employment_intensity_distribution <- function(dt) {
  ...
  # Add percentages
  intensity_dist[,
    percentage_of_periods := round(100 * n_periods / sum(n_periods), 1)
  ]
  intensity_dist[,
    percentage_of_days := round(100 * total_days / sum(total_days), 1)
  ]
...
#' @keywords internal
#' @noRd
.calculate_temporal_overlap_patterns <- function(
  dt,
  temporal_aggregation = "month"
) {
  ...
  if (!inherits(dt$inizio, "Date") || !inherits(dt$fine, "Date")) {
    warning(
      "Date columns not properly formatted for temporal analysis. Skipping temporal patterns."
    )
    return(NULL)
  }
  ...
  if (temporal_aggregation == "month") {
    ...
    overlap_periods[,
      temporal_period := paste0(format(inizio, "%Y"), "-Q", quarter(inizio))
    ]
    overlap_periods[,
      temporal_label := paste0("Q", quarter(inizio), " ", format(inizio, "%Y"))
    ]
  ...
  temporal_patterns[,
    percentage_of_overlaps := round(
      100 * n_overlapping_periods / sum(n_overlapping_periods),
      1
    )
  ]
  temporal_patterns[,
    percentage_of_overlap_days := round(
      100 * total_overlap_days / sum(total_overlap_days),
      1
    )
  ]
...
#' @keywords internal
#' @noRd
.calculate_group_overlap_comparisons <- function(
  dt,
  group_by,
  include_unemployment_context = TRUE
) {
  ...
  group_comparisons <- dt[,
    {
      ...
      pct_persons_with_overlaps := if (total_persons > 0) {
        100 * persons_with_overlaps / total_persons
      } else {
        0
      }
      ...
      avg_employment_intensity <- if (total_employment_periods > 0) {
        mean(arco[arco >= 1], na.rm = TRUE)
      } else {
        0
      }
      max_employment_intensity <- if (total_employment_periods > 0) {
        max(arco[arco >= 1], na.rm = TRUE)
      } else {
        0
      }
      ...
      list(
        ...
        avg_overlap_duration = round(avg_overlap_duration, 1),
        employment_to_unemployment_ratio = if (
          is.na(employment_to_unemployment_ratio)
        ) {
          NA_real_
        } else {
          round(employment_to_unemployment_ratio, 2)
  ...
  group_comparisons[,
    overlap_frequency_rank := rank(-overlap_frequency, ties.method = "min")
  ]
  group_comparisons[,
    overlap_intensity_rank := rank(-overlap_intensity_pct, ties.method = "min")
  ]
  group_comparisons[,
    persons_with_overlaps_rank := rank(
      -pct_persons_with_overlaps,
      ties.method = "min"
    )
  ]
...
#' @keywords internal
#' @noRd
.create_empty_overlap_results <- function(
  analysis_level,
  group_by = NULL,
  include_temporal_patterns = FALSE
) {
  ...
  if (analysis_level == "detailed") {
    ...
    results$employment_intensity_distribution <- data.table(
      arco = integer(0),
      n_periods = integer(0),
      percentage_of_periods = numeric(0)
    )

    if (include_temporal_patterns) {
      results$temporal_patterns <- data.table(
        temporal_period = character(0),
        n_overlapping_periods = integer(0),
        percentage_of_overlaps = numeric(0)
      )
    }
    ...
    if (!is.null(group_by)) {
      ...
      setnames(
        ...
        names(results$group_comparisons),
  ...
  person_results <- dt[,
    {
      ...
      list(
        ...
        total_unemployment_days = total_unemployment_days,
        employment_to_unemployment_ratio = if (total_unemployment_days > 0) {
          round(total_employment_days / total_unemployment_days, 2)
        } else {
          NA_real_
        }
      )
...
#' @keywords internal
#' @noRd
.create_no_overlap_results <- function(
  analysis_level,
  dt,
  person_results = NULL,
  group_by = NULL,
  ...
  if (analysis_level %in% c("aggregate", "detailed")) {
    ...
    results$aggregate_overlaps <- list(
      ...
      employment_intensity_breakdown = if (total_employment_periods > 0) {
        data.table(
          arco = 1L,
          n_periods = total_employment_periods,
          percentage = 100.0,
          employment_type = "Single employment"
        )
      } else {
        data.table(
          arco = integer(0),
...
#' @keywords internal
#' @noRd
.extract_global_state_space <- function(
  pipeline_result,
  transition_variable,
  eval_chain
) {
...
#' @keywords internal
#' @noRd
.generate_time_periods <- function(
  pipeline_result,
  time_column,
  time_format,
  custom_period_days,
  ...
  if (time_format == "monthly") {
    ...
    period_boundaries <- c(
      period_starts,
      seq.Date(
        from = period_starts[length(period_starts)],
        length.out = 2,
        by = "month"
      )[2]
    )
    ...
    start_quarter <- as.Date(paste0(
      format(min_date, "%Y"),
      "-",
      sprintf("%02d", (as.numeric(format(min_date, "%m")) - 1) %/% 3 * 3 + 1),
      "-01"
    ))

    # Generate sequence of quarter starts
    quarter_months <- seq(
      from = as.numeric(format(start_quarter, "%m")),
      to = 12 + as.numeric(format(max_date, "%m")),
      by = 3
    )
    quarter_years <- rep(
      as.numeric(format(start_quarter, "%Y")):as.numeric(format(
        max_date,
        "%Y"
      )),
      each = 4
    ...
    quarter_dates <- as.Date(paste0(
      quarter_years[valid_quarters],
      "-",
      sprintf("%02d", quarter_months[valid_quarters]),
      "-01"
    ))
    ...
    if (
      is.null(custom_period_days) ||
        !is.numeric(custom_period_days) ||
        custom_period_days <= 0
    ) {
      stop(
        "custom_period_days must be a positive integer when time_format is 'custom'"
      )
    }

    # Create custom period boundaries
    period_starts <- seq.Date(
      from = min_date,
      to = max_date,
      by = custom_period_days
    )
    period_boundaries <- c(
      period_starts,
      period_starts[length(period_starts)] + custom_period_days
    )
...
#' @export
create_monthly_transition_matrices <- function(
  pipeline_result,
  transition_variable = NULL,
  time_column = "fine",
  ...
  if (
    matrix_format == "sparse" && !requireNamespace("Matrix", quietly = TRUE)
  ) {
    stop(
      "Package 'Matrix' is required for sparse matrix format but not installed."
    )
  }
  ...
  if (is.null(transition_variable)) {
    ...
    if (length(available_vars) == 0) {
      stop(
        "No suitable transition variable found. Please specify transition_variable parameter."
      )
    }
    transition_variable <- available_vars[1]
    if (show_progress) {
      message(sprintf(
        "Using '%s' as transition variable (auto-detected).",
        transition_variable
      ))
    }
  }
  ...
  if (consolidation_type != "none" && !"over_id" %in% names(pipeline_result)) {
    stop(
      "Parameter 'pipeline_result' must contain 'over_id' column from vecshift() output when consolidation_type is not 'none'."
    )
  }
  ...
  if (show_progress) {
    ...
    message(sprintf(
      "Dataset contains %d rows across %d individuals",
  ...
  global_states <- .extract_global_state_space(
    pipeline_result,
    transition_variable,
    eval_chain
  )
  ...
  if (show_progress) {
    ...
    states_preview <- if (n_states > 5) {
      paste0(states_preview, "...")
    } else {
      states_preview
    }
  ...
  if (estimated_memory_gb > memory_limit_gb && use_global_state_space) {
    if (show_progress) {
      message(sprintf(
        "WARNING: Estimated memory usage (%.2f GB) exceeds limit (%.2f GB)",
        estimated_memory_gb,
        memory_limit_gb
      ))
      message(
        "Automatically switching to period-specific state spaces for memory efficiency..."
      )
    }
    use_global_state_space <- FALSE
  }
  ...
  if (n_states > 10000 && matrix_format == "dense") {
    if (show_progress) {
      message(sprintf(
        "Large state space (%d states) detected. Switching to sparse matrix format.",
        n_states
      ))
    }
    matrix_format <- "sparse"
    if (!requireNamespace("Matrix", quietly = TRUE)) {
      stop(
        "Package 'Matrix' is required for large state spaces but not installed. Please install it or reduce the state space size."
      )
    }
  }
  ...
  if (show_progress) {
    if (use_global_state_space) {
      ...
      message(
        "Step 3: Creating transition matrices with period-specific state spaces..."
      )
    }
  }
  ...
  if (consolidation_type != "none") {
    ...
    pipeline_result_converted <- .convert_types_optimized(
      pipeline_result,
    ...
    if (exists("merge_consecutive_employment", mode = "function")) {
      ...
      if (
        exists("merge_consecutive_employment", envir = asNamespace("vecshift"))
      ) {
        merge_func <- get(
          "merge_consecutive_employment",
          envir = asNamespace("vecshift")
        )
        ...
        if (show_progress) {
          message(
            "Warning: merge_consecutive_employment function not found. Proceeding without consolidation."
          )
        }
      ...
      if (show_progress) {
        message(
          "Warning: vecshift package not available for consolidation. Proceeding without consolidation."
        )
      }
      consolidated_data <- pipeline_result_converted
    }
  ...
  if (length(multi_period_people) == 0) {
    if (show_progress) {
      message(
        "No people with multiple employment periods found - no transitions possible"
      )
    }
  ...
  potential_transitions[,
    unemployment_duration := as.numeric(inizio - prev_fine - 1)
  ]
  ...
  if (nrow(valid_transitions) == 0) {
    ...
    all_transitions_dt <- data.table(
      ...
      from = character(0),
      to = character(0),
      to_date = as.Date(character(0)),
      unemployment_duration = numeric(0)
    )
  } else {
    # Process chain values vectorized (much more efficient)
    from_processed <- .process_chain_value(
      valid_transitions$prev_transition_var,
      eval_chain
    )
    to_processed <- .process_chain_value(
      valid_transitions[[transition_variable]],
      eval_chain
    )
  ...
  all_transitions_dt[,
    unemployment_duration := as.double(unemployment_duration)
  ]

  if (show_progress) {
    message(sprintf(
      "Extracted %d total transitions across all time periods",
      nrow(all_transitions_dt)
    ))
  }
  ...
  for (i in 1:n_periods) {
    ...
    if (show_progress && (i == 1 || i %% 5 == 0 || i == n_periods)) {
      message(sprintf(
        "Processing period %d/%d: %s (%s to %s)",
        i,
        n_periods,
        period_names[i],
    ...
    period_transitions <- all_transitions_dt[
      to_date >= period_start & to_date <= period_end
    ]
    ...
    aggregated_transitions <- period_transitions[,
      .(weight = .N),
      by = .(from, to)
    ]
    ...
    if (use_global_state_space) {
      ...
      if (nrow(aggregated_transitions) > 0) {
        ...
        from_indices <- match(
          as.character(aggregated_transitions$from),
          global_states
        )
        to_indices <- match(
          as.character(aggregated_transitions$to),
          global_states
        )
        ...
        if (any(valid_idx)) {
          ...
          for (k in seq_along(from_indices)) {
            period_matrix[from_indices[k], to_indices[k]] <- period_matrix[
              from_indices[k],
              to_indices[k]
            ] +
              weights[k]
      ...
      unique_states_period <- sort(unique(c(
        aggregated_transitions$from,
        aggregated_transitions$to
      )))
      ...
      if (nrow(aggregated_transitions) > 0) {
        ...
        for (k in 1:nrow(aggregated_transitions)) {
          period_matrix[from_indices[k], to_indices[k]] <- period_matrix[
            ...
            aggregated_transitions$weight[k]
        }
      }
    }
  ...
  if (matrix_type == "probability") {
    ...
    for (i in 1:n_periods) {
      if (
        !is.null(matrices_list[[i]]) &&
          (is.matrix(matrices_list[[i]]) ||
            inherits(matrices_list[[i]], "Matrix"))
      ) {
        matrices_list[[i]] <- .normalize_transition_matrix(
          matrices_list[[i]],
          normalize_by
        )
      }
  ...
  if (show_progress) {
    if (use_global_state_space) {
      message(sprintf(
        "Completed in %.2f seconds. Created %d matrices (%d with transitions) of %dx%d dimensions.",
        as.numeric(elapsed_time),
        n_periods,
        periods_with_transitions,
        n_states,
        n_states
      ))
    } else {
      # Calculate average matrix dimensions for period-specific approach
      actual_dims <- lapply(matrices_list, function(m) {
        if (is.matrix(m) || inherits(m, "Matrix")) dim(m) else c(0, 0)
      })
      avg_rows <- mean(sapply(actual_dims, function(d) d[1]), na.rm = TRUE)
      avg_cols <- mean(sapply(actual_dims, function(d) d[2]), na.rm = TRUE)
      message(sprintf(
        "Completed in %.2f seconds. Created %d matrices (%d with transitions). Average dimensions: %.0fx%.0f.",
        as.numeric(elapsed_time),
        n_periods,
        periods_with_transitions,
        avg_rows,
  ...
  if (include_summary) {
    results$metadata <- list(
      ...
      matrix_dimensions = if (use_global_state_space) {
        c(n_states, n_states)
      } else {
        "variable (period-specific)"
      },
...
#' @importFrom Matrix Matrix sparseMatrix
#' @export
create_monthly_transition_matrices_optimized <- function(
  pipeline_result,
  transition_variable = NULL,
  time_column = "fine",
  time_format = c("monthly", "quarterly", "custom"),
  ...
  if (is.null(transition_variable)) {
    ...
    if (length(potential_vars) == 0) {
      stop(
        "No suitable transition variable found. Specify transition_variable explicitly."
      )
    }
    ...
    if (show_progress) {
      message(sprintf(
        "Auto-detected transition variable: %s",
        transition_variable
      ))
    }
  }
  ...
  if (!transition_variable %in% names(pipeline_result)) {
    stop(sprintf(
      "Transition variable '%s' not found in data",
      transition_variable
    ))
  }
  ...
  if (time_format == "monthly") {
    ...
    period_boundaries <- c(
      period_boundaries,
      seq(
        period_boundaries[length(period_boundaries)],
        length.out = 2,
        by = "month"
      )[2]
    )
    ...
    start_quarter <- as.Date(paste0(
      format(date_range[1], "%Y-"),
      c("01-01", "04-01", "07-01", "10-01")[ceiling(
        as.numeric(format(date_range[1], "%m")) / 3
      )]
    ))
    end_quarter <- as.Date(paste0(
      format(date_range[2], "%Y-"),
      c("01-01", "04-01", "07-01", "10-01")[ceiling(
        as.numeric(format(date_range[2], "%m")) / 3
      )]
    ))
    ...
    period_boundaries <- seq(
      date_range[1],
      date_range[2],
      by = paste(custom_period_days, "days")
    )
    if (tail(period_boundaries, 1) < date_range[2]) {
      period_boundaries <- c(
        period_boundaries,
        tail(period_boundaries, 1) + custom_period_days
      )
    }
  }
  ...
  if (name_format == "date") {
    if (time_format == "monthly") {
      period_names <- tolower(format(
        period_boundaries[-length(period_boundaries)],
        "%b%Y"
      ))
    } else if (time_format == "quarterly") {
      period_names <- paste0(
        format(period_boundaries[-length(period_boundaries)], "%Y"),
        "q",
        quarters(period_boundaries[-length(period_boundaries)])
      )
    ...
    if (length(custom_names) != n_periods) {
      stop(sprintf(
        "custom_names length (%d) must match number of periods (%d)",
        length(custom_names),
        n_periods
      ))
    }
  ...
  if (show_progress) {
    message(
      "Step 2: Analyzing state space and estimating memory requirements..."
    )
  }
  ...
  if (show_progress) {
    message(sprintf(
      "Estimated memory for global state space: %.2f GB (%d states, %d periods)",
      estimated_memory_gb,
      n_states,
      n_periods
    ))
  ...
  if (use_global_state_space && estimated_memory_gb > memory_limit_gb) {
    if (show_progress) {
      message(sprintf(
        "WARNING: Estimated memory (%.2f GB) exceeds limit (%.2f GB).",
        estimated_memory_gb,
        memory_limit_gb
      ))
      message(
      ...
      message(
        "Tip: Increase memory_limit_gb if you have sufficient RAM and need consistent dimensions."
      )
    }
    use_global_state_space <- FALSE
  }
  ...
  if (n_states > 10000 && matrix_format != "sparse") {
    if (show_progress) {
      message(sprintf(
        "Large state space detected (%d states). Automatically switching to sparse matrix format.",
        n_states
      ))
    }
    matrix_format <- "sparse"
    ...
    if (use_global_state_space) {
      ...
      if (nrow(aggregated_transitions) > 0) {
        ...
        if (any(valid_idx)) {
          ...
          period_matrix[as.integer(names(
            aggregated_weights
          ))] <- aggregated_weights
        }
      }
