...
#'
#' @section Deprecated in v0.9.0:
#' Use [career_profile()] with `indicators = "core"` (and optionally
#' `"stability"`) instead. Scheduled for removal in v1.0.0.
#'
#' @keywords internal
#' @export
calculate_career_success_metrics <- function(
  data,
  survival_data = NULL,
  id_column = "cf",
  ...
  .Deprecated(
    ...
    package = "longworkR",
    msg = paste0(
      "calculate_career_success_metrics() is deprecated in longworkR v0.9.0. ",
      "Use career_profile(indicators = 'core') and career_profile(",
      "indicators = c('core', 'stability')) instead. ",
  ...
  valid_rows <- !is.na(data[["over_id"]]) &
    !is.na(data[["durata"]]) &
    data[["over_id"]] > 0 &
    data[["durata"]] >= min_spell_duration
  ...
  if (!is.null(survival_data) && "median_survival" %in% names(survival_data)) {
    ...
    valid_medians <- median_survivals[
      !is.na(median_survivals) & median_survivals > 0
    ]
    ...
    if (length(valid_medians) > 0) {
      ...
      quality_scores <- if (max_survival > min_survival) {
        0.1 +
          0.9 * (valid_medians - min_survival) / (max_survival - min_survival)
      } else {
    ...
    contract_durations <- data[
      valid_rows,
      .(median_duration = fmedian(durata)),
      by = c(contract_col)
    ]
    ...
    if (max_dur > min_dur) {
      contract_durations[,
        quality_score := 0.1 +
          0.9 * (median_duration - min_dur) / (max_dur - min_dur)
      ]
    } else {
      contract_durations[, quality_score := 0.5]
    }

    quality_scores <- setNames(
      contract_durations$quality_score,
      contract_durations$contract_code
    )
  }
  ...
  intensity_scores <- ifelse(
    is.na(data[[intensity_col]]),
    0.5,
    pmax(0, pmin(1, data[[intensity_col]] / 3))
  )
    ...
    career_index <- data[valid_rows][,
      {
        ...
        high_quality_exposure <- sum(durata_vals[high_quality_mask]) /
          total_days
        ...
        if (unique_contracts > 1) {
          ...
          contract_days_dt <- data.table(
            contract = contract_codes,
            days = durata_vals
          )
          contract_totals <- contract_days_dt[,
            .(total_days = sum(days)),
            by = contract
          ]
          contract_proportions <- contract_totals$total_days / total_days
          contract_entropy <- -sum(
            contract_proportions * log(contract_proportions + 1e-10)
          )
          max_entropy <- log(unique_contracts)
          diversity_component <- if (max_entropy > 0) {
            contract_entropy / max_entropy
          } else {
            0
          }
        } else {
        ...
        growth_opportunity_score <- pmax(
          0,
          pmin(1, 0.6 * high_quality_exposure + 0.4 * diversity_component)
        )
        ...
        if (length(durata_vals) <= 1) {
          ...
          quality_improvements <- sum(
            to_quality > from_quality * 1.05,
            na.rm = TRUE
          )
          intensity_improvements <- sum(
            to_intensity > from_intensity * 1.05,
            na.rm = TRUE
          )
          ...
          career_advancement_index <- 1 /
            (1 + exp(-6 * (advancement_rate - 0.3)))
        }
        ...
        if (enhance_variability) {
          ...
          career_success_index <- sqrt(pmin(
            1,
            (contract_enh^0.35) *
              (intensity_enh^0.25) *
              (stability_enh^0.25) *
              (growth_opportunity_score^0.15)
          ))
        } else {
          career_success_index <- pmax(
            0,
            pmin(
              1,
              0.35 *
                avg_contract_quality +
                0.25 * avg_intensity_score +
                0.25 * stability_score +
                0.15 * growth_opportunity_score
            )
          )
        }
    ...
    career_index <- data[valid_rows][,
      {
        ...
        contract_codes <- get(contract_col)
        unique_contracts <- uniqueN(contract_codes)
        high_quality_exposure <- sum(durata_vals[contract_quality_vals > 0.7]) /
          total_days
        ...
        if (enhance_variability) {
          career_success_index <- sqrt(pmin(
            1,
            (sqrt(avg_contract_quality)^0.35) *
              ((avg_intensity_score^1.5)^0.25) *
              ((1 / (1 + exp(-4 * (stability_score - 0.5))))^0.25) *
              (growth_opportunity_score^0.15)
          ))
        } else {
          career_success_index <- pmax(
...
#'
#' @section Deprecated in v0.9.0:
#' Use [career_profile()] with `indicators = c("core", "transitions")`
#' instead. Scheduled for removal in v1.0.0.
#'
#' @keywords internal
#' @export
calculate_career_transition_metrics <- function(
  data,
  survival_data = NULL,
  id_column = "cf",
  time_period_column = NULL,
  ...
  .Deprecated(
    ...
    package = "longworkR",
    msg = paste0(
      "calculate_career_transition_metrics() is deprecated in longworkR ",
      "v0.9.0. Use career_profile(indicators = c('core', 'transitions')) ",
      "instead. This function will be removed in v1.0.0."
    )
  )
  if (!inherits(data, "data.table")) {
    stop("Input data must be a data.table")
  ...
  if (!is.null(survival_data) && "median_survival" %in% names(survival_data)) {
    ...
    contract_durations <- setNames(
      contract_durations$median_duration,
      contract_durations$contract_code
    )
  }
  ...
  if (is.null(time_period_column)) {
    transition_metrics <- data[valid_rows][
      order(get(id_column), get(date_column)),
      {
        n_obs <- .N
        if (n_obs <= 1) {
          ...
          duration_improvements <- sum(
            to_duration_exp > from_duration_exp * 1.1,
            na.rm = TRUE
          )
          duration_deteriorations <- sum(
            to_duration_exp < from_duration_exp * 0.9,
            na.rm = TRUE
          )
          fulltime_improvements <- sum(
            !from_fulltime & to_fulltime,
            na.rm = TRUE
          )
          fulltime_deteriorations <- sum(
            from_fulltime & !to_fulltime,
            na.rm = TRUE
          )
          ...
          if (has_salary) {
            ...
            salary_improvements <- sum(
              (to_salary > from_salary * 1.05) & valid_salary_mask,
              na.rm = TRUE
            )
            salary_deteriorations <- sum(
              (to_salary < from_salary * 0.95) & valid_salary_mask,
              na.rm = TRUE
            )
            total_improvements <- duration_improvements +
          ...
          if (has_salary) {
            progression_components <- c(
              ...
              salary_improvements / n_transitions
            )
          }
    ...
    transition_metrics <- data[valid_rows][
      order(get(id_column), get(time_period_column), get(date_column)),
      {
        n_obs <- .N
        if (n_obs <= 1) {
          base_result <- list(
...
#'
#' @section Deprecated in v0.9.0:
#' Use [career_profile()] with `indicators = c("core", "stability")`
#' instead. Scheduled for removal in v1.0.0.
#'
#' @keywords internal
#' @export
...
calculate_career_stability_metrics <- function(
  data,
  id_column = "cf",
  time_period_column = NULL,
  date_column = "inizio",
  ...
  .Deprecated(
    ...
    package = "longworkR",
    msg = paste0(
      "calculate_career_stability_metrics() is deprecated in longworkR ",
      "v0.9.0. Use career_profile(indicators = c('core', 'stability')) ",
      "instead. This function will be removed in v1.0.0."
    )
  )
  if (!inherits(data, "data.table")) {
      ...
      order(get(id_column), get(date_column)),
      {
        ...
        if (n_obs <= 1) {
          ...
          for (i in seq_len(n_spells)) {
            spell_durations[i] <- sum(durata_vals[
              spell_ids == unique_spells[i]
            ])
          }
          ...
          avg_emp_spell <- if (n_emp_spells > 0) {
            fmean(emp_spell_durations)
          } else {
            0.0
          }
          avg_unemp_spell <- if (n_unemp_spells > 0) {
            fmean(unemp_spell_durations)
          } else {
            0.0
          }
          max_emp_spell <- if (n_emp_spells > 0) {
            fmax(emp_spell_durations)
          } else {
            0.0
          }
          max_unemp_spell <- if (n_unemp_spells > 0) {
            fmax(unemp_spell_durations)
          } else {
            0.0
          }
...
#'
#' @section Deprecated in v0.9.0:
#' Use [career_profile()] with `indicators = c("core", "complexity")`
#' instead. Scheduled for removal in v1.0.0.
#'
#' @keywords internal
#' @export
#' @importFrom collapse fmean fmax
calculate_career_complexity_metrics <- function(
  data,
  id_column = "cf",
  time_period_column = NULL,
  ...
  .Deprecated(
    ...
    package = "longworkR",
    msg = paste0(
      "calculate_career_complexity_metrics() is deprecated in longworkR ",
      "v0.9.0. Use career_profile(indicators = c('core', 'complexity')) ",
      "instead. This function will be removed in v1.0.0."
    )
  )
  if (!inherits(data, "data.table")) {
  ...
  if (!is.null(time_period_column) && time_period_column %in% names(data)) {
    ...
    data_subset <- data[,
      c(
        ...
        "over_id",
        available_vars,
        if (has_inizio) "inizio" else NULL
      ),
      with = FALSE
    ]
    data_subset[, time_period := "overall"]
    ...
    complexity_metrics <- data_subset[,
      {
        ...
        if (n_emp_obs == 0) {
          ...
          diversity_vars <- intersect(
            available_vars,
            c("prior", "COD_TIPOLOGIA_CONTRATTUALE", "qualifica", "ateco")
          )
          ...
          raw_score <- concurrent_component +
            rate_component +
            diversity_component +
    ...
    complexity_metrics <- data[,
      {
        ...
        if (n_emp_obs == 0) {
          ...
          if (.N <= 1) {
            ...
            transitions <- sum(abs(diff(emp_status)))
            period_years <- sum(durata) / 365.25
            fragmentation_idx <- pmin(
              1.0,
              (transitions / pmax(1.0, period_years)) / 4.0
            )
          }
...
#'
#' @section Deprecated in v0.9.0:
#' Use [career_profile()] with `indicators = "all"` instead. Scheduled for
#' removal in v1.0.0.
#'
#' @keywords internal
#' @export
calculate_comprehensive_career_metrics <- function(
  data,
  survival_data = NULL,
  metrics = "all",
  id_column = "cf",
  ...
  .Deprecated(
    ...
    package = "longworkR",
    msg = paste0(
      "calculate_comprehensive_career_metrics() is deprecated in longworkR ",
      "v0.9.0. Use career_profile(indicators = 'all') instead. ",
      "This function will be removed in v1.0.0."
    )
  )
  .assert_vecshift_input(
    data,
    required_cols = c("cf", "over_id", "durata")
  )
  ...
  if (
    ...
    if (output_format == "list") {
      ...
      base_cols <- if (is.null(time_period_column)) {
        id_column
      } else {
        c(id_column, time_period_column)
      }

      if ("quality" %in% metrics) {
        avail_quality_cols <- intersect(quality_cols, names(unified_result))
        metric_results$quality <- unified_result[,
          c(base_cols, avail_quality_cols),
          with = FALSE
        ]
      }
      ...
      if ("stability" %in% metrics) {
        ...
        metric_results$stability <- unified_result[,
          c(base_cols, avail_stability_cols),
          with = FALSE
        ]
      }

      if ("complexity" %in% metrics) {
        avail_complexity_cols <- intersect(
          complexity_cols,
          names(unified_result)
        )
        metric_results$complexity <- unified_result[,
          c(base_cols, avail_complexity_cols),
          with = FALSE
        ]
      }

      if ("transitions" %in% metrics && include_transitions) {
        transition_cols <- setdiff(
          names(unified_result),
          c(base_cols, quality_cols, stability_cols, complexity_cols)
        )
        if (length(transition_cols) > 0) {
          metric_results$transitions <- unified_result[,
            c(base_cols, transition_cols),
            with = FALSE
          ]
        }
      }
    ...
    id_vars <- if (is.null(time_period_column)) {
      id_column
    } else {
      c(id_column, time_period_column)
    }
  ...
  merge_cols <- if (is.null(time_period_column)) {
    id_column
  } else {
    c(id_column, time_period_column)
  }
...
#' @keywords internal
calculate_unified_career_metrics_optimized <- function(
  data,
  survival_data = NULL,
  id_column = "cf",
  time_period_column = NULL,
  ...
  if (exists("is_temp_period") && is_temp_period) {
    result <- data_work[
      order(get(id_column)),
      {
        ...
        unique_contracts <- uniqueN(contract_codes)
        high_quality_exposure <- sum(durata_vals[quality_vals > 0.7]) /
          total_days

        if (unique_contracts > 1) {
          contract_days_agg <- data.table(
            contract = contract_codes,
            days = durata_vals
          )[, .(total = sum(days)), by = contract]
          contract_proportions <- contract_days_agg$total / total_days
          diversity_component <- -sum(
            contract_proportions * log(contract_proportions + 1e-10)
          ) /
            log(unique_contracts)
        } else {
          diversity_component <- 0
        }
        ...
        if (n_obs <= 1) {
          ...
          spell_durations <- vapply(
            unique_spells,
            function(id) sum(durata_vals[spell_ids == id]),
            numeric(1)
          )
          ...
          employment_spells <- as.double(n_emp_spells)
          unemployment_spells <- as.double(n_unemp_spells)
          avg_employment_spell <- if (n_emp_spells > 0) {
            fmean(emp_spell_durations)
          } else {
            0.0
          }
          avg_unemployment_spell <- if (n_unemp_spells > 0) {
            fmean(unemp_spell_durations)
          } else {
            0.0
          }
          max_employment_spell <- if (n_emp_spells > 0) {
            fmax(emp_spell_durations)
          } else {
            0.0
          }
          max_unemployment_spell <- if (n_unemp_spells > 0) {
            fmax(unemp_spell_durations)
          } else {
            0.0
          }

          job_turnover_rate <- n_emp_spells /
            pmax(total_days / 365.25, 1 / 365.25)
        }

        employment_security_index <- pmin(
          1,
          (0.4 *
            employment_rate +
        ...
        if ("prior" %in% complexity_variables && emp_days > 0) {
          ...
          if (length(unique_types) > 1) {
            type_counts <- tabulate(match(prior_vals, unique_types))
            type_props <- type_counts / length(prior_vals)
            employment_diversity_index <- -sum(
              type_props * log(type_props + 1e-10)
            )
          } else {
            employment_diversity_index <- 0.0
          }
        ...
        concurrent_component <- 0.25 *
          pmin(1, sqrt(pmax(0, max_concurrent_jobs - 1)))
        rate_component <- 0.25 /
          (1 + exp(-15 * (concurrent_employment_rate - 0.05)))
        ...
        career_complexity_index <- sqrt(pmin(
          1,
          concurrent_component +
            rate_component +
            diversity_component +
    ...
    return(data[valid_rows][
      order(get(id_column), get(time_period_column)),
      {
