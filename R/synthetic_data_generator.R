...
generate_synthetic_employment_data <- function(
  ...
  contracts_per_individual <- rpois(
    n_individuals,
    lambda = n_contracts / n_individuals
  )
  ...
  if (total_contracts != n_contracts) {
    ...
    if (diff > 0) {
      ...
      for (i in add_indices) {
        contracts_per_individual[i] <- contracts_per_individual[i] + 1
      }
    } else {
      # Remove contracts
      remove_indices <- sample(
        which(contracts_per_individual > 1),
        abs(diff),
        replace = TRUE
      )
  ...
  for (i in 1:n_individuals) {
    ...
    start_dates <- sort(sample(
      seq(start_date, end_date - 30, by = "day"),
      n_contracts_individual,
      replace = TRUE
    ))
    ...
    individual_contracts <- data.table(
      ...
      over_id = sample(
        c(0, over_id_counter:(over_id_counter + n_contracts_individual)),
        n_contracts_individual,
        replace = TRUE
      ),
      stato = sample(
      ...
      ateco = sample(
        ...
        n_contracts_individual,
        replace = TRUE,
        prob = c(0.3, rep(0.05, 14))
      ),
      ore = sample(
        c(NA, 4, 8, 12, 15, 20, 25, 27, 30, 35, 40, 48),
        n_contracts_individual,
        replace = TRUE,
        prob = c(
          0.3,
      ...
      eta = individual_data$eta_base +
        sample(-2:5, n_contracts_individual, replace = TRUE),
    ...
    individual_contracts[,
      event_start := ifelse(
        did_attribute == 1,
        as.Date("2022-01-01") +
          sample(0:500, sum(did_attribute), replace = TRUE),
        NA
      )
    ]
    individual_contracts[,
  ...
  validation_results <- list(
    ...
    structural_checks = list(
      has_required_columns = all(
        c("cf", "inizio", "fine", "durata", "stato") %in% names(synthetic_data)
      ),
      valid_dates = all(
        synthetic_data$inizio <= synthetic_data$fine,
        na.rm = TRUE
      ),
      positive_duration = all(synthetic_data$durata > 0, na.rm = TRUE),
      reasonable_ages = all(
        synthetic_data$eta >= 16 & synthetic_data$eta <= 70,
        na.rm = TRUE
      )
    )
  )
