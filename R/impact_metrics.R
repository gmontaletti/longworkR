...
# 0. Internal helper for the v0.9.0 migration window -----
#
# The impact functions below delegate to `calculate_career_*_metrics()`,
# which were marked deprecated in longworkR v0.9.0 (W2). Until the
# impact suite is refactored to consume `career_profile()` directly
# (which would require adding per-period grouping to `career_profile()`
# or changing the public column contract of the impact functions), we
# silence ONLY the targeted `.Deprecated()` warning so callers of the
# impact API do not see leaked W2 warnings. Any other warning still
# propagates normally.
#
# Scheduled removal: when `career_profile()` gains a `time_period_column`
# argument or when the impact functions migrate to a new column contract.
.call_silencing_w2_deprecation <- function(expr) {
  withCallingHandlers(
    expr,
    warning = function(w) {
      msg <- conditionMessage(w)
      if (grepl("deprecated in longworkR", msg, fixed = TRUE)) {
...
#'
#' @export
calculate_employment_stability_metrics <- function(
  data,
  id_column = "cf",
  ...
  # The underlying function is deprecated in v0.9.0; silence ONLY that
  # targeted warning here (see `.call_silencing_w2_deprecation` above).
  stability_metrics <- .call_silencing_w2_deprecation(
  ...
  if (
    period_column != "period" && period_column %in% names(stability_metrics)
  ) {
...
#'
#' @export
calculate_contract_quality_metrics <- function(
  data,
  id_column = "cf",
  period_column = "event_period",
  contract_code_column = "COD_TIPOLOGIA_CONTRATTUALE",
  ...
  setnames(
    dt,
    c(id_column, period_column, contract_code_column),
  ...
  if (!is.null(internship_codes) && length(internship_codes) > 0) {
    ...
    quality_metrics <- dt[,
      .(
        permanent_contract_days = as.numeric(sum(
          durata[is_permanent == TRUE],
          na.rm = TRUE
        )),
        temporary_contract_days = as.numeric(sum(
          durata[is_temporary == TRUE],
          na.rm = TRUE
        )),
        internship_contract_days = as.numeric(sum(
          durata[is_internship == TRUE],
          na.rm = TRUE
        )),
        total_employment_days = as.numeric(sum(durata, na.rm = TRUE)),
        average_contract_quality = as.numeric(fmean(
  ...
  if (!is.null(internship_codes) && length(internship_codes) > 0) {
    ...
    quality_metrics[, `:=`(
      ...
      internship_contract_rate = internship_contract_days /
        total_employment_days,
      temporary_contract_rate = temporary_contract_days / total_employment_days
    )]
    quality_metrics[
      is.nan(permanent_contract_rate),
      permanent_contract_rate := 0
    ]
    quality_metrics[
      is.nan(internship_contract_rate),
      internship_contract_rate := 0
    ]
    quality_metrics[
      is.nan(temporary_contract_rate),
      temporary_contract_rate := 0
    ]
  } else {
    # Two-tier rates
    quality_metrics[,
      permanent_contract_rate := permanent_contract_days /
  ...
  transition_metrics <- dt[
    order(cf, period, inizio),
    {
      if (.N <= 1) {
        ...
        if (!is.null(internship_codes) && length(internship_codes) > 0) {
          ...
          temp_to_perm <- as.numeric(sum(
            contract_changes$from %in%
              temporary_codes &
              contract_changes$to %in% permanent_codes,
            na.rm = TRUE
          ))
          perm_to_temp <- as.numeric(sum(
            contract_changes$from %in%
              permanent_codes &
              contract_changes$to %in% temporary_codes,
            na.rm = TRUE
          ))
          temp_to_internship <- as.numeric(sum(
            contract_changes$from %in%
              temporary_codes &
              contract_changes$to %in% internship_codes,
            na.rm = TRUE
          ))
          internship_to_perm <- as.numeric(sum(
            contract_changes$from %in%
              internship_codes &
              contract_changes$to %in% permanent_codes,
            na.rm = TRUE
  ...
  contract_metrics[,
    contract_improvement_rate := pmax(0, temp_to_perm_transitions) /
      pmax(1, contract_observations / 4)
  ]
...
#'
#' @export
calculate_impact_career_complexity_metrics <- function(
  data,
  id_column = "cf",
  period_column = "event_period",
  ...
  # The underlying function is deprecated in v0.9.0; silence ONLY that
  # targeted warning here (see `.call_silencing_w2_deprecation` above).
  complexity_metrics <- .call_silencing_w2_deprecation(
  ...
  # Rename period column to match expected output
  if (
    period_column != "period" && period_column %in% names(complexity_metrics)
  ) {
...
#'
#' @export
calculate_transition_pattern_metrics <- function(
  data,
  id_column = "cf",
  period_column = "event_period",
  date_column = "inizio"
  ...
  setnames(
    dt,
    c(id_column, period_column, date_column),
    c("cf", "period", "start_date")
  )
    ...
    order(cf, period, start_date),
    {
      if (.N <= 1) {
        ...
        if (nrow(emp_periods) >= 2) {
          ...
          job_gaps <- emp_periods[2:.N, start_date] -
            emp_periods[1:(.N - 1), end_date]
          job_gaps <- as.numeric(job_gaps[job_gaps > 0]) # Only positive gaps

          avg_gap <- if (length(job_gaps) > 0) {
            as.double(fmean(job_gaps, na.rm = TRUE))
          } else {
            NA_real_
          }
          median_gap <- if (length(job_gaps) > 0) {
            as.double(fmedian(job_gaps, na.rm = TRUE))
          } else {
            NA_real_
          }
        } else {
        ...
        successful_searches <- if (unemp_spells > 0) {
          ...
          unemp_to_emp <- as.double(sum(
            diff(as.integer(employed)) > 0,
            na.rm = TRUE
          ))
        ...
        employment_transitions <- as.double(sum(
          diff(as.integer(employed)) != 0,
          na.rm = TRUE
        ))
...
#'
#' @keywords internal
calculate_career_success_index_for_impact <- function(
  metric_results,
  id_column,
  period_column
) {
  ...
  if ("quality" %in% names(metric_results)) {
    contract_quality_component <- metric_results$quality[, .(
      cf,
      period,
      contract_quality = fifelse(
        is.na(average_contract_quality),
        0.5,
  ...
  if ("stability" %in% names(metric_results)) {
    stability_component <- metric_results$stability[, .(
      cf,
      period,
      stability_score = fifelse(
        is.na(employment_stability_index),
        0.5,
  ...
  if ("stability" %in% names(metric_results)) {
    intensity_component <- metric_results$stability[, .(
      cf,
      period,
  ...
  if ("quality" %in% names(metric_results)) {
    growth_component <- metric_results$quality[, .(
      cf,
      period,
      growth_score = fifelse(
        is.na(contract_improvement_rate),
        0.3,
  ...
  if (
    !is.null(stability_component) &&
      is.null(career_success_data[["stability_score"]])
  ) {
    career_success_data <- merge(
      career_success_data,
      stability_component,
...
#'
#' @export
calculate_comprehensive_impact_metrics <- function(
  data,
  metrics = "all",
  ...
  if (career_metrics) {
    ...
    # The underlying function is deprecated in v0.9.0; silence ONLY that
    # targeted warning here (see `.call_silencing_w2_deprecation` above).
    career_metrics_result <- .call_silencing_w2_deprecation(
    ...
    if (nrow(career_metrics_result) > 0) {
      ...
      if (length(unique_periods) > 1) {
        ...
        career_expanded <- career_metrics_result[rep(
          seq_len(nrow(career_metrics_result)),
          length(unique_periods)
        )]
        career_expanded[,
          period := rep(unique_periods, each = nrow(career_metrics_result))
        ]
    ...
    for (metric_name in names(metric_results)) {
      if (metric_name != "career") {
        overlapping_cols <- intersect(
          names(metric_results[[metric_name]]),
          career_metric_cols
        )
