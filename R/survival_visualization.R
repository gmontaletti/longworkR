...
visualize_contract_survival <- function(
  ...
  p <- ggplot(
    plot_data,
    aes(x = time, y = survival_prob, color = contract_type)
  ) +
    ...
    for (ct in unique(plot_data$contract_type)) {
      ...
      for (i in 1:nrow(ct_data)) {
        ...
        if (i < nrow(ct_data)) {
          ...
          # W3.3: copy() removed — ct_data[i] already allocates a fresh dt.
          step_point <- ct_data[i]
  ...
  if (is.character(color_palette) && length(color_palette) == 1) {
    ...
    if (color_palette %in% rownames(RColorBrewer::brewer.pal.info)) {
      ...
      p <- p +
        scale_color_manual(values = colors, name = "Contract Type") +
        scale_fill_manual(
          values = colors,
          name = "Contract Type",
          guide = "none"
      ...
      warning(sprintf(
        "Color palette '%s' not found in RColorBrewer. Using default ggplot2 colors.",
        color_palette
      ))
    }
    ...
    is.vector(color_palette) &&
      length(color_palette) > 0 &&
      length(color_palette) < n_colors
  ) {
    # Extend insufficient custom color vector
    warning(sprintf(
      "Color palette has %d colors but %d needed. Extending palette.",
...
plot_survival_comparison <- function(
  ...
  if (is.logical(comparison_results) && comparison_results == TRUE) {
    ...
    if (requireNamespace("survival", quietly = TRUE)) {
      ...
      warning(
        "comparison_results = TRUE requires actual comparison data. Pass the output from compare_contract_survival() instead."
      )
    } else {
      comparison_results <- NULL
    }
  ...
  if (!is.null(groups_to_show)) {
    ...
    warning(sprintf(
      "Showing only first %d contract types. Use 'groups_to_show' parameter to specify which groups to display.",
      max_groups
    ))
  ...
  filtered_survival_curves$survival_tables <- survival_curves$survival_tables[
    contract_types
  ]
  filtered_survival_curves$survival_fits <- survival_curves$survival_fits[
    contract_types
  ]
  filtered_survival_curves$median_survival <- survival_curves$median_survival[
    contract_types
  ]
  if (!is.null(survival_curves$confidence_intervals)) {
    filtered_survival_curves$confidence_intervals <- survival_curves$confidence_intervals[
      contract_types
    ]
  }
  ...
  p <- visualize_contract_survival(
    ...
    subtitle = if (
      !is.null(comparison_results) &&
        !is.na(comparison_results$test_results$logrank$chisq)
    ) {
      sprintf(
        "Log-rank p-value: %.4f",
        1 -
...
plot_survival_heatmap <- function(
  ...
  if (is.null(time_grid)) {
    max_times <- sapply(survival_curves$survival_tables, function(x) {
      max(x$time)
    })
  ...
  p <- ggplot(
    heatmap_data,
    aes(x = time, y = contract_type, fill = survival_prob)
  ) +
    geom_tile() +
    labs(
  ...
  if (color_scale == "viridis") {
    ...
    if (utils::packageVersion("ggplot2") >= "3.0.0") {
      ...
      if (requireNamespace("viridis", quietly = TRUE)) {
        ...
        warning(
          "Viridis color scale not available. Using blue-purple gradient as fallback."
        )
        p <- p +
          scale_fill_gradient(
            low = "#440154",
...
plot_median_survival_forest <- function(
      ...
      tryCatch(
        {
          ...
          if (!is.na(median_val) && !is.na(lower_ci) && !is.na(upper_ci)) {
            ...
            if (lower_ci > median_val || upper_ci < median_val) {
              warning(sprintf(
                "Contract type '%s': Invalid confidence intervals (lower: %.2f, median: %.2f, upper: %.2f). Setting to NA.",
                ct,
                lower_ci,
                median_val,
                upper_ci
        ...
        error = function(e) {
          warning(sprintf(
            "Contract type '%s': Cannot extract confidence intervals: %s",
            ct,
            e$message
          ))
        }
  ...
  if (order_by == "median") {
    ...
    ordered_levels <- c(
      intersect(custom_order, all_types),
      setdiff(all_types, custom_order)
    )
    forest_data[,
      contract_type := factor(contract_type, levels = ordered_levels)
    ]
    forest_data <- forest_data[order(contract_type)]
  }
  ...
  missing_ci_count <- sum(
    is.na(forest_data$lower_ci) | is.na(forest_data$upper_ci)
  )
  if (missing_ci_count > 0) {
    p <- p +
      labs(
        caption = sprintf(
          "Note: Confidence intervals not available for %d contract type(s)",
          missing_ci_count
        )
      )
  }
...
validate_animation_setup <- function() {
  ...
  if (!capabilities("png")) {
    stop(
      "PNG graphics device is not available. This is required for animations."
    )
  }
  ...
  test_file <- file.path(
    temp_dir,
    paste0("gganimate_test_", Sys.getpid(), ".txt")
  )
  tryCatch(
    {
  ...
  if (gganimate_version < "1.0.0") {
    warning(
      "gganimate version ",
      gganimate_version,
      " may have compatibility issues. Version >= 1.0.0 recommended."
    )
  }
  ...
  if (!has_working_renderer) {
    stop(
      "No suitable animation renderer found. Install 'gifski' or 'magick' package."
    )
  }
...
animate_survival_curves <- function(
  ...
  if (!is.list(survival_curves) || is.null(survival_curves$survival_tables)) {
    stop(
      "Invalid survival_curves input. Expected output from estimate_contract_survival()"
    )
  }
  ...
  if (total_points > 5000) {
    ...
    message(sprintf(
      "Large dataset detected (%d points). Using adaptive sampling (every %d points, max %d frames).",
      total_points,
      sample_every,
      max_frames
    ))
  ...
  if (length(all_times) > max_frames) {
    time_indices <- c(
      1,
      round(seq(2, length(all_times), length.out = max_frames - 1))
    )
  ...
  if (sample_every > 1 && length(selected_times) > sample_every) {
    keep_indices <- c(
      1,
      seq(sample_every, length(selected_times), by = sample_every),
  ...
  message(sprintf(
    "Creating animation with %d frames from %d data points",
    actual_frames,
    nrow(anim_data)
  ))
  ...
  p <- ggplot2::ggplot(
    anim_data,
    ggplot2::aes(x = time, y = survival_prob, color = contract_type)
  ) +
  ...
  tryCatch(
    ...
    error = function(e) {
      ...
      warning(
        "Could not create dedicated temp directory, using default: ",
        e$message
      )
    }
  )
  ...
  if (is.null(anim)) {
    tryCatch(
      ...
      error = function(e) {
        ...
        stop(
          "Unable to create animation. Please check gganimate installation and system graphics capabilities. Last error: ",
          e$message
        )
      }
    )
        ...
        tryCatch(
          {
            ...
            message(sprintf(
              "Animation saved to temporary location: %s",
              temp_file
            ))
          ...
          error = function(e2) {
            warning(
              "Failed to save animation even to temp directory: ",
              e2$message
            )
