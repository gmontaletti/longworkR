...
.get_temporal_period <- function(
  ...
  if (period_type == "quarterly") {
    ...
    period_num <- (period_year - ref_year) *
      4 +
      (period_quarter - ref_quarter) +
      1
...
#' @keywords internal
.compute_employment_rate <- function(
  data,
  period_type = "quarterly",
  id_var = "cf",
  ...
  work_data[,
    period_start := .get_temporal_period(inizio, period_type, reference_date)
  ]
  work_data[,
    period_end := .get_temporal_period(fine, period_type, reference_date)
  ]
  ...
  work_data[
    period_start > period_end_limited,
    period_end_limited := period_start
  ]
  ...
  for (i in seq_len(nrow(work_data))) {
    ...
    contract_period_list[[i]] <- data.table(
      ...
      period_end_limited_orig = rep(
        row$period_end_limited,
        length(periods_spanned)
      ),
  ...
  employment_records[,
    days_employed := pmax(0, as.numeric(contract_end - contract_start + 1))
  ]
  ...
  period_employment[,
    employment_rate := pmin(1.0, total_days_employed / period_duration)
  ]

  # Apply employment threshold
  period_employment[
    total_days_employed < employment_threshold,
    employment_rate := 0
  ]

  return(period_employment[, .(
    cf,
    period,
    period_start,
    period_end,
...
#' @keywords internal
.compute_contract_quality <- function(
  data,
  period_type = "quarterly",
  id_var = "cf",
  ...
  tryCatch(
          ...
          if (max_median > min_median) {
            quality_scores <- sapply(median_survivals, function(survival_time) {
              if (is.na(survival_time)) {
                ...
                0.3 +
                  0.7 * (survival_time - min_median) / (max_median - min_median)
              }
            })
          } else {
    ...
    error = function(e) {
      ...
      warning(
        "Kaplan-Meier survival analysis failed, falling back to duration-based quality scores: ",
        e$message
      )
      warning(
        "This may occur with insufficient data or when survival package is unavailable."
      )
      ...
      avg_duration_by_type <- work_data[,
      ...
      if (length(avg_durations) > 0) {
        ...
        if (max_duration > min_duration) {
          quality_scores <- 0.3 +
            0.7 * (avg_durations - min_duration) / (max_duration - min_duration)
        } else {
  ...
  work_data[,
    intensity_factor := ifelse(
      is.na(employment_intensity) | employment_intensity == 1,
      1.0,
      0.8
    )
  ]
  ...
  quality_records[,
    days_in_period := pmax(0, as.numeric(contract_end - contract_start + 1))
  ]
  ...
  quality_records[,
    weighted_permanent := as.numeric(contract_type == "A.01.00") *
      days_in_period
  ]
  ...
  quality_by_period <- quality_records[,
    .(
      avg_contract_quality = sum(weighted_quality, na.rm = TRUE) /
        sum(days_in_period, na.rm = TRUE),
      total_employment_days = sum(days_in_period, na.rm = TRUE),
      n_contracts = length(unique(row_id)),
      pct_permanent = sum(weighted_permanent, na.rm = TRUE) /
        sum(days_in_period, na.rm = TRUE),
      pct_full_time = sum(weighted_full_time, na.rm = TRUE) /
        sum(days_in_period, na.rm = TRUE),
...
#'
#' @export
compute_temporal_employment_indicators <- function(
  data,
  period_type = "quarterly",
  indicators = c("employment_rate", "contract_quality", "stability_index"),
  id_var = "cf",
  ...
  if (!is.null(quality_weights)) {
    warning(
      "The 'quality_weights' parameter is deprecated and will be ignored. ",
  ...
  if ("employment_rate" %in% indicators) {
    if (verbose) {
      cat("Computing employment rates...\n")
    }
  ...
  if ("contract_quality" %in% indicators) {
    if (verbose) {
      cat("Computing contract quality...\n")
    }
    ...
    if (contract_type_var %in% names(data)) {
      ...
      if (verbose) {
        cat(
          "Contract quality function returned",
          nrow(contract_quality),
          "rows\n"
        )
        cat(
          "Contract quality periods:",
          paste(sort(unique(contract_quality$period)), collapse = ", "),
          "\n"
        )
      ...
      warning(
        ...
        contract_type_var,
        "' not found. Skipping contract quality."
      )
    }
  }
  ...
  if (
    "wage_statistics" %in%
      indicators &&
      !is.null(wage_var) &&
      wage_var %in% names(data)
  ) {
    if (verbose) {
      cat("Computing wage statistics...\n")
    }
    ...
    wage_data[,
      period := .get_temporal_period(inizio, period_type, reference_date)
    ]
  ...
  if ("stability_index" %in% indicators) {
    ...
    all_periods <- unique(c(
      stability_data$period_start,
      stability_data$period_end
    ))
    period_info <- .create_period_ranges(
      all_periods,
      period_type,
      reference_date
    )
    ...
    expanded_stability_data <- merge(
      expanded_stability_data,
      ...
      stability_metrics <- expanded_stability_data[,
        {
          ...
          if (n_contracts > 1L) {
            ...
            duration_cv <- if (duration_mean > 0) {
              sqrt(var(durata, na.rm = TRUE)) / duration_mean
            } else {
              0.0
            }
          ...
          frequency_score <- pmax(
            0.0,
            1.0 - pmin(1.0, abs(n_contracts - 2L) / 5.0)
          )
  ...
  if ("contract_types" %in% indicators && contract_type_var %in% names(data)) {
    if (verbose) {
      cat("Computing contract type distributions...\n")
    }
    ...
    top_contracts <- contract_data[, .N, by = contract_type][order(-N)][
      1:5
    ]$contract_type
    ...
    contract_distributions <- contract_data[,
      {
        ...
        contract_summary <- .SD[,
          .(duration = sum(durata, na.rm = TRUE)),
          by = contract_type
        ]
        ...
        for (contract in top_contracts) {
          ...
          contract_duration <- contract_summary[
            contract_type == contract,
            duration
          ]
          if (length(contract_duration) == 0) {
            contract_duration <- 0
          }
          type_percentages[[pct_var]] <- ifelse(
            total_duration > 0,
            contract_duration / total_duration,
            0
          )
        }
  ...
  if ("transition_patterns" %in% indicators) {
    if (verbose) {
      cat("Computing transition patterns...\n")
    }
    ...
    results$transition_patterns <- employment_by_period[, .(
      cf,
      period,
      has_employment,
      transition_type
  ...
  if (
    "employment_rate" %in% indicators && "employment_rates" %in% names(results)
  ) {
    # W3.3: copy() removed — merge() below returns a new object on the first
    # iteration; no in-place mutation occurs before that.
    final_results <- results$employment_rates
    first_result_name <- "employment_rates"
  } else if (length(results) > 0) {
    first_result_name <- names(results)[1]
    final_results <- results[[first_result_name]]
  } else {
    ...
    if (indicator != first_result_name) {
      ...
      if (length(duplicate_cols) > 0) {
        for (dup_col in duplicate_cols) {
          ...
          if (
            x_col %in% names(final_results) && y_col %in% names(final_results)
          ) {
            ...
            if (base_name %in% c("n_contracts", "total_days_employed")) {
              ...
              final_results[,
                (base_name) := ifelse(is.na(get(x_col)), get(y_col), get(x_col))
              ]
            }
  ...
  if (output_format == "wide") {
    ...
    value_cols <- setdiff(
      names(final_results),
      c(id_cols, "period", "period_start", "period_end")
    )
  ...
  if (verbose) {
    ...
    if ("period" %in% names(final_results)) {
      cat(
        "Periods covered:",
        min(final_results$period, na.rm = TRUE),
        "to",
...
#'
#' @export
create_temporal_indicators_summary <- function(
  temporal_data,
  group_var = NULL,
  indicator_vars = NULL,
  ...
  if (is.null(indicator_vars)) {
    ...
    indicator_vars <- setdiff(
      numeric_cols,
      c("cf", "period", "period_start", "period_end")
    )
  }
  ...
  for (indicator in indicator_vars) {
    if (
      indicator %in%
        names(temporal_data) &&
        is.numeric(temporal_data[[indicator]])
    ) {
      # Calculate requested statistics
      indicator_summary <- temporal_data[
        !is.na(get(indicator)),
        {
          result_list <- list()
