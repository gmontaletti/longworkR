#' Consolidation Helper Functions for longworkR Package
#'
#' @description
...
#'       \itemize{
#'         \item{\code{unique_employers_original}}: Vector of unique employer IDs before consolidation
#'         \item{\code{unique_employers_consolidated}}: Vector of unique employer IDs after consolidation
#'         \item{\code{n_unique_employers_original}}: Count of unique employers before consolidation
#'         \item{\code{n_unique_employers_consolidated}}: Count of unique employers after consolidation
#'         \item{\code{employer_consolidation_details}}: Per-employer consolidation metrics data.table
...
#'         \item{\code{employer_gaps}}: Gap analysis for employer transitions
#'       }
...
.generate_consolidated_data_for_metrics <- function(original_data, consolidation_mode, 
  ...
  # PHASE 2 OPTIMIZATION: Efficient on-demand consolidation
  # This solves the major wrapper function issue by generating actual consolidated data
...
#' the same logic as consolidate_by_employer() but avoiding expensive consolidation
...
.apply_employer_consolidation_simple <- function(data, employer_var, min_lag = 30) {
  ...
  # Use the existing consolidate_by_employer function if available
  # Otherwise return a simplified version
  if (exists("consolidate_by_employer", mode = "function")) {
    return(consolidate_by_employer(data, employer_var, min_lag))
  } else {
    # Fallback: return original data with warning
    warning("consolidate_by_employer function not available, returning original data")
    return(data)
  }
}
...
.apply_temporal_consolidation_simple <- function(data, cons_type = "both") {
  ...
  # Use the enhanced consolidate_by_temporal function if available
  # This provides consistent behavior with the main consolidation function
  if (exists("consolidate_by_temporal", mode = "function")) {
    return(consolidate_by_temporal(
      data, 
    ...
    # Fallback: PHASE 2 OPTIMIZATION: Fix type consistency issues before consolidation
    data_copy <- data.table::copy(data)
    
    # Convert potential integer columns to numeric to prevent type conflicts
    for (col_name in names(data_copy)) {
      col_class <- class(data_copy[[col_name]])[1]
      if (col_class == "integer" && col_name != "cf") {
        # Keep cf as integer for person ID, convert others to numeric
        data.table::set(data_copy, j = col_name, value = as.numeric(data_copy[[col_name]]))
      }
    }
    
    # Use existing temporal consolidation functions if available
    if (exists("merge_consecutive_employment_fast_with_type", mode = "function")) {
      return(merge_consecutive_employment_fast_with_type(data_copy, consolidation_type = cons_type))
      ...
      return(merge_consecutive_employment(data_copy, consolidation_type = cons_type))
    } else {
      warning("No temporal consolidation function available, returning original data")
      return(data_copy)
    }
  }
}
...
#' print(metrics_employer$employer_specific$unique_employers_original)
#' print(metrics_employer$employer_specific$n_unique_employers_original)
...
print.longworkR_consolidation_summary <- function(x, ...) {
  ...
  if (!is.null(x$person_summary)) {
    ...
    # Add employment percentage if available
    if (!is.null(x$distribution_summary) && !is.null(x$distribution_summary$avg_employment_percentage)) {
      cat(sprintf("  Average employment coverage: %.1f%%\n", x$distribution_summary$avg_employment_percentage))
    }
    cat("\n")
  }
