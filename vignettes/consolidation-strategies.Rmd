...
The longworkR package provides **three focused functions** for employment consolidation, each with a single, clear purpose:

1. **`consolidate_overlapping()`** - Merges concurrent employment periods
2. **`consolidate_adjacent()`** - Merges touching employment periods (no gap)
3. **`consolidate_short_gaps()`** - Bridges short unemployment gaps
...
The three consolidation functions are designed to be used **in sequence**, creating a progressive consolidation chain:
...
3. **Short gaps last**: Most aggressive, should work on already-simplified data
...
The old `employer_var` parameter for employer-based consolidation is **not directly supported** in the new API. This functionality may be restored in a future release or requires custom implementation:
...
- [ ] Consider if employer-based consolidation was needed (requires workaround)
...
- Function documentation: `?consolidate_overlapping`, `?consolidate_adjacent`, `?consolidate_short_gaps`
