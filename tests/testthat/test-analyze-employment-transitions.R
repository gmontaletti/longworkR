...
test_that("employer-based consolidation validates parameters correctly", {
  ...
  expect_error(
    ...
    "Parameter 'employer_var' is required when consolidation_mode = 'employer'"
  )
  ...
  expect_error(
    ...
    "Parameter 'consolidation_mode' must be one of: employer, temporal, none, both"
  )
...
test_that("employer-based consolidation works with basic same-employer scenarios", {
  ...
  if (nrow(result) > 0) {
    ...
    expect_true(any(grepl("CompanyA.*CompanyB", transitions, ignore.case = TRUE)))
  }
})
...
test_that("employer-based consolidation never consolidates different employers", {
  ...
  if (nrow(result_employer) > 0 && nrow(result_temporal) > 0) {
    ...
    employer_transitions <- paste(result_employer$from, "->", result_employer$to)
...
test_that("employer-based consolidation respects min_employer_lag parameter", {
  ...
  if (nrow(result_strict) > 0 && nrow(result_moderate) > 0 && nrow(result_permissive) > 0) {
...
test_that("employer-based consolidation preserves data integrity", {
  ...
  test_data <- data.table(
    ...
    company = c("CompanyA", "CompanyA", "CompanyB", "CompanyC",
               "CompanyD", "CompanyD", "CompanyE"),
    employer_id = c("EMP001", "EMP001", "EMP002", "EMP003",
                   "EMP004", "EMP004", "EMP005"),
    salary = c(50000, 55000, 60000, 65000, 45000, 48000, 52000),
    department = c("IT", "IT", "Finance", "HR", "Marketing", "Marketing", "Sales")
  )
...
test_that("employer-based consolidation handles multiple people correctly", {
  ...
  test_data <- data.table(
    ...
    inizio = as.Date(c("2023-01-01", "2023-03-01", "2023-06-01",  # Person 1
                       "2023-01-15", "2023-04-01", "2023-07-01",  # Person 2
                       "2023-02-01", "2023-08-01")),              # Person 3
    fine = as.Date(c("2023-02-28", "2023-05-31", "2023-08-31",    # Person 1
                     "2023-03-15", "2023-06-30", "2023-09-30",    # Person 2
                     "2023-07-31", "2023-12-31")),                # Person 3
    arco = c(1, 1, 1, 1, 1, 1, 1, 1),
    durata = c(58, 92, 92, 60, 91, 92, 181, 153),
    company = c("CompanyA", "CompanyA", "CompanyB",              # P1: same->diff
               "CompanyC", "CompanyD", "CompanyD",              # P2: diff->same
               "CompanyE", "CompanyE"),                         # P3: same->same
    employer_id = c("EMP001", "EMP001", "EMP002",               # P1: same->diff
                   "EMP003", "EMP004", "EMP004",               # P2: diff->same
                   "EMP005", "EMP005")                         # P3: same->same
  )
...
test_that("consolidation_mode parameter validation works correctly", {
  ...
  expect_error(
    ...
    "Parameter 'employer_var' is required when consolidation_mode = 'both'"
  )
...
test_that("consolidation_mode 'both' combines employer and temporal consolidation", {
  ...
  if (!exists("merge_consecutive_employment_fast_with_type", mode = "function")) {
    skip("Temporal consolidation function not available for 'both' mode testing")
  }
  
  # Create test data where 'both' mode should produce different results than individual modes
  test_data <- data.table(
    cf = c(rep("PERSON001", 6)),
    inizio = as.Date(c("2023-01-01", "2023-02-01", "2023-02-15", "2023-04-01", "2023-04-15", "2023-06-01")),
    fine = as.Date(c("2023-01-31", "2023-02-10", "2023-03-31", "2023-04-10", "2023-05-31", "2023-08-31")),
    arco = c(1, 1, 1, 1, 1, 1),
    durata = c(31, 10, 45, 10, 47, 92),
    over_id = c(100, 101, 101, 102, 102, 103),
    company = c("CompanyA", "CompanyA", "CompanyB", "CompanyB", "CompanyC", "CompanyD"),
  ...
  if (nrow(result_none) > 0) expect_true(all(required_cols %in% names(result_none)))
  if (nrow(result_temporal) > 0) expect_true(all(required_cols %in% names(result_temporal)))
  if (nrow(result_employer) > 0) expect_true(all(required_cols %in% names(result_employer)))
  if (nrow(result_both) > 0) expect_true(all(required_cols %in% names(result_both)))
})
...
test_that("consolidation modes preserve data integrity with sample data", {
  ...
  if (nrow(test_data) > 0) {
    ...
    if (exists("merge_consecutive_employment_fast_with_type", mode = "function")) {
      ...
      results_to_check <- list(result_none, result_employer, result_temporal, result_both)
    }
