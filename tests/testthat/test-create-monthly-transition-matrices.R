...
test_that("create_monthly_transition_matrices basic functionality", {
  ...
  matrix_dims <- lapply(result$matrices, function(m) {
    if (is.null(m)) c(0, 0) else dim(m)
  })
  expect_true(all(sapply(matrix_dims, function(d) {
    length(d) == 2 && all(d >= 0)
  })))
...
test_that("create_monthly_transition_matrices metadata", {
  ...
  # Create test data: two employment periods separated by an unemployment gap
  # so that internal consolidation does not merge them into one period.
  # over_id=0 marks the unemployment gap; consolidation_type="none" bypasses
  # the internal merge step so the transition is always detectable.
  test_data <- data.table(
    cf = c(1, 1, 1),
    arco = c(1, 0, 1),
    ...
    over_id = c(1, 0, 2),
    prior = c(0, 0, 0),
    COD_TIPOLOGIA_CONTRATTUALE = c("A.01.00", NA_character_, "A.02.00")
  )

  # Test with metadata included; consolidation_type="none" prevents internal
  # merging so both employment periods survive and generate one transition.
  result <- create_monthly_transition_matrices(
    test_data,
    transition_variable = "COD_TIPOLOGIA_CONTRATTUALE",
