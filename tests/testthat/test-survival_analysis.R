...
create_test_employment_data <- function(n_persons = 100, seed = 123) {
  ...
  dt <- data.table(
    ...
    INIZIO = as.Date("2020-01-01") +
      sample(0:365, n_persons * 2, replace = TRUE),
...
test_that("confidence intervals are valid", {
  ...
  valid_ci <- !is.na(result$median_ci_lower) &
    !is.na(result$median_ci_upper) &
    !is.na(result$contract_type_median)

  if (any(valid_ci)) {
    expect_true(all(
      result$median_ci_lower[valid_ci] <= result$contract_type_median[valid_ci]
    ))
    expect_true(all(
      result$contract_type_median[valid_ci] <= result$median_ci_upper[valid_ci]
    ))
...
test_that("export_survival_results creates files", {
  skip_if_not(
    interactive(),
    "Skipping file creation test in non-interactive mode"
  )
...
test_that("survival analysis integrates with vecshift output", {
  ...
  # Add survival metrics; vecshift output uses lowercase column names
  survival_result <- add_contract_survival_metrics(
    dt,
    start_var = "inizio",
    end_var = "fine"
  )
