...
create_sample_temporal_data <- function(
  n_persons = 3,
  n_contracts_per_person = 4
) {
  set.seed(123)
  ...
  for (i in 1:n_persons) {
    ...
    contracts_start <- base_date +
      cumsum(c(0, sample(30:120, n_contracts_per_person - 1)))
    ...
    person_contracts <- data.table(
      ...
      COD_TIPOLOGIA_CONTRATTUALE = sample(
        c("A.01.00", "A.03.00", "A.03.01", "C.01.00", "A.07.00"),
        n_contracts_per_person,
        replace = TRUE
      ),
      prior = sample(
        c(0, 1),
      ...
      company = paste0(
        "Company",
        sample(LETTERS[1:10], n_contracts_per_person, replace = TRUE)
      )
    )
...
test_that("compute_temporal_employment_indicators returns correct structure with quarterly periods", {
  ...
  sample_data <- create_sample_temporal_data(
    n_persons = 2,
    n_contracts_per_person = 3
  )
...
test_that("compute_temporal_employment_indicators handles contract consolidation with over_id", {
  ...
  expect_true(all(
    result_consolidated$employment_rate >= 0 &
      result_consolidated$employment_rate <= 1
  ))
  expect_true(all(
    result_not_consolidated$employment_rate >= 0 &
      result_not_consolidated$employment_rate <= 1
  ))
})
...
test_that("compute_temporal_employment_indicators handles multiple indicators including contract quality", {
  ...
  if ("avg_contract_quality" %in% names(result)) {
    quality_values <- result$avg_contract_quality[
      !is.na(result$avg_contract_quality)
    ]
...
test_that("create_temporal_indicators_summary returns correct group summaries", {
  ...
  temporal_indicators[,
    treatment_group := rep(
      c("control", "treatment"),
      length.out = nrow(temporal_indicators)
    )
  ]
