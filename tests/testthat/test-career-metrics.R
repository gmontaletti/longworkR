test_that("calculate_career_success_metrics works with basic data", {
  ...
  expect_true(all(
    result$career_success_index >= 0 & result$career_success_index <= 1
  ))
  expect_true(all(
    result$career_advancement_index >= 0 & result$career_advancement_index <= 1
  ))
  expect_true(all(
    result$contract_quality_score >= 0 & result$contract_quality_score <= 1
  ))
...
test_that("calculate_career_transition_metrics handles salary data", {
  ...
  expect_s3_class(result, "data.table")
  expect_true(all(
    c("salary_improvements", "salary_deteriorations") %in% names(result)
  ))
...
test_that("calculate_career_success_metrics works with comprehensive index", {
  ...
  expect_true(all(
    result$career_stability_score >= 0 & result$career_stability_score <= 1
  ))
})
...
test_that("calculate_comprehensive_career_metrics integrates all metrics", {
  ...
  dt <- data.table(
    ...
    fine = as.Date(rep(
      c("2020-05-30", "2020-10-29", "2021-05-30", "2021-10-29"),
      2
    )),
  ...
  quality_cols <- grep(
    "career_success_index|career_advancement_index|contract_quality|intensity|stability_score|growth_opportunity",
    names(result_wide),
    value = TRUE
  )
  transition_cols <- grep(
    "transition|improvement|advancement",
    names(result_wide),
    value = TRUE
  )
  stability_cols <- grep(
    "employment_rate|employment_spells|turnover|employment_stability",
    names(result_wide),
    value = TRUE
  )
  ...
  expect_true(all(
    c("cf", "metric_name", "metric_value", "metric_category") %in%
      names(result_long)
  ))
  expect_true(all(
    unique(result_long$metric_category) %in% c("quality", "stability", "other")
  ))
})
...
test_that("career metrics handle edge cases", {
  ...
  quality_result <- calculate_career_success_metrics(
    dt_single,
    survival_data = survival_data_single
  )
  ...
  comprehensive_result <- calculate_career_success_metrics(
    dt_single,
    survival_data = survival_data_single
  )
...
test_that("career metrics validate input parameters", {
  ...
  expect_error(
    calculate_career_success_metrics(
      as.data.frame(dt),
      survival_data = survival_data_basic
    ),
...
test_that("employment diversity index varies with different employment types", {
  ...
  dt <- data.table(
    ...
    prior = c(
      ...
      1,
      0,
      1, # some diversity (2 types)
      0,
      1,
      2,
      3 # high diversity (4 types)
    )
  )
...
test_that("complexity_variables parameter works with different variables", {
  ...
  result_prior <- calculate_career_complexity_metrics(
    dt,
    complexity_variables = c("prior")
  )
  expect_equal(result_prior$employment_diversity_index, 0.0)

  # Test with contract types (should be > 0)
  result_contract <- calculate_career_complexity_metrics(
    dt,
    complexity_variables = c("COD_TIPOLOGIA_CONTRATTUALE")
  )
  expect_true(result_contract$employment_diversity_index > 0)

  # Test with qualifica (should be 0)
  result_qual <- calculate_career_complexity_metrics(
    dt,
    complexity_variables = c("qualifica")
  )
