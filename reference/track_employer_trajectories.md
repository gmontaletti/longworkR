...
contract type, following their employer changes for 4 quarters (91 days
each) after the end date of their chronologically first occurrence of
that contract type.
...
- employer_col:
  ...
  Character. Column name containing employer codes (required parameter,
  no default)
...
- palette:
  ...
  Character. Color palette to use: "employer", "main", "desaturated",
  "bw" (default: "employer")
...
- `data`: data.table with individual-level quarterly employer
  trajectories
...
start date) as a reference point, then tracks employer transitions in
the subsequent quarters. Employer status is determined by comparing the
quarter's employer with the reference contract's employer.
...
- Within each quarter, the employer is taken from the contract with the
  latest end date
...
latest end date provides the employer code.
